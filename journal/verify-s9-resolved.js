...
// ── UI IS A DIRECTORY NOW, NOT A FILE (repaired after `ddjp_397`) ───────────────────────────
// `ui/interface.js` was split into eleven files at `ddjp_397` and DELETED. Two checks below read
// it by name, so THIS WHOLE VERIFIER HAS THROWN ENOENT ON LINE 1 EVER SINCE — every check in it,
// including Q, which is the one that asserts HANDOFF's heading agrees with the tree's `?v=` tags.
// Q is exactly the instrument that would have caught the package whose heading said `?v=367` while
// the tree carried `?v=393`, and it was dead for the entire time that drift existed.
//
// It went unnoticed because `tests/run-all.js` discovers `tests/check-*.js` ONLY — this file lives
// in `journal/` and is run by hand. A guard that crashes outside the suite does not report a
// failure; it reports nothing at all, which is worse than red.
//
// REPAIRED BY DERIVING RATHER THAN RENAMING: these read the ui/ DIRECTORY, so the next split moves
// them with it instead of breaking them again. Naming a successor file would have bought exactly
// one split's worth of working.
const uiFiles = () => fs.readdirSync(path.join(APP, "ui"))
  .filter((f) => f.endsWith(".js")).sort();
const uiAll = () => uiFiles().map((f) => app("ui/" + f)).join("\n");
const uiAllCode = () => uiFiles().map((f) => code("ui/" + f)).join("\n");
...
{
  const built = /_openLibraryIO|addToPlaylist|add-to-playlist/i.test(uiAll());
  ...
  check("F", "Grab destinations are BUILT and `04-features.md` already describes the picker.",
    "grep ui/*.js for the picker declarations; grep 04-features.md for the description",
...
// ── S — rule D holds: ui/ does not name the reducer, and the fold lives in features/room.js ────
{
  ...
  check("S", "J13 was built against the corrected field and never went near rule D: the fold lives in `features/room.js` and `ui/` does not name `StateDeriver`.",
    "grep COMMENT-STRIPPED ui/*.js for StateDeriver; grep features/room.js for foldActivity",
...
// ── U — RULED 2A (ddjp_423): ui/ MAY read ServerClock; the two statements saying otherwise are gone.
{
  const claims = [app("ui/player.js"), app("tests/check-playhead.js")]
    .some((x) => /ServerClock is on ui\/'s forbidden list|ui\/ may not touch ServerClock/.test(x));
  const cb = app("tests/check-boundaries.js").match(/const UI_FORBIDDEN\s*=\s*\[[^\]]*\]/);
  const listed = !!cb && /ServerClock/.test(cb[0]);
  check("U", "Owner ruled ui/ may read ServerClock: neither ui/player.js nor check-playhead still claims rule D forbids it, and rule D's list does not name it.",
    "grep ui/player.js and tests/check-playhead.js for the claim; read UI_FORBIDDEN in check-boundaries.js",
    (!claims && !listed) ? "HOLDS" : "FAILS", "claim-still-present=" + claims + " ServerClock-in-UI_FORBIDDEN=" + listed);
}

// ── V — RULED 3B (ddjp_423): rule D is left as it is. The ruling stands while the list is unchanged.
{
  const cb = app("tests/check-boundaries.js").match(/const UI_FORBIDDEN\s*=\s*\[[^\]]*\]/);
  const unchanged = !!cb && !/\bRanks\b/.test(cb[0]) && /StreamManager/.test(cb[0]);
  check("V", "Owner ruled rule D stays as it is: its list still names the interface globals and still omits Ranks, the gap this entry records.",
    "read UI_FORBIDDEN in check-boundaries.js",
    unchanged ? "HOLDS" : "FAILS", "list-as-recorded=" + unchanged + (unchanged ? "" : " (rule D has changed; re-read this entry)"));
}

// ── W — FIXED (ddjp_423): Room History's "ago" and the loader's fallback measure in SERVER time.
{
  const panels = code("ui/panels.js");
  const hist = /const now = \(typeof ServerClock !== "undefined" && ServerClock\.serverNow\) \? ServerClock\.serverNow\(\) : Date\.now\(\);[\s\S]{0,4000}_fmtAgo\(h\.at, now\)/.test(panels);
  const player = /elapsed = \(\(\(typeof ServerClock !== "undefined" && ServerClock\.serverNow\) \? ServerClock\.serverNow\(\) : Date\.now\(\)\) - w\.startedAt\) \/ 1000/.test(code("ui/player.js"));
  check("W", "Room History hands _fmtAgo a ServerClock now, and the loader's fallback subtracts startedAt from ServerClock rather than the device clock.",
    "read ui/panels.js for the now assignment ahead of _fmtAgo(h.at, now); read ui/player.js's fallback",
    (hist && player) ? "HOLDS" : "FAILS", "history=" + hist + " loader-fallback=" + player);
}

// ── X — CORRECTED IN THE DOCS (ddjp_424): four live descriptions the tree contradicted. ────────
{
  const spec = !/Nothing sends it today/.test(doc("consensus/consensus-models.md"));
  const arch = doc("main/02-architecture.md");
  const layer = !/never reaches past the one directly/.test(arch);
  const map = !/backend2\/\s+empty stub/.test(arch) && !/interface, chatbuffer/.test(arch);
  const header = !/exposes TWO/.test(app("tests/check-boundaries.js").slice(0, 3000));
  const called = /BotSettings\.request\s*\(/.test(code("ui/settings.js"));
  check("X", "The four stale descriptions are gone: the spec no longer says no UI sends a bot request (ui/settings.js calls BotSettings.request), 02-architecture's layer sentence and code map match the tree, and check-boundaries' header no longer claims TWO interface globals.",
    "grep consensus-models.md, 02-architecture.md and check-boundaries.js's header for each old claim; read ui/settings.js for the call",
    (spec && layer && map && header && called) ? "HOLDS" : "FAILS",
    "spec=" + spec + " layer=" + layer + " map=" + map + " header=" + header + " request-still-called=" + called);
}
