...
const Ranks = (() => {
  ...
  // ── HOW A RUNG IS WRITTEN FOR A PERSON TO READ (J36) ────────────────────────────────────────
  // One row per rung, beside the ladder, for the reason the two per-rank tables above are here:
  // it resizes with the ladder, and a rung renamed in one place would otherwise keep an old label
  // in another. Keyed by NAME, never by level, so a rung that MOVES carries its label with it —
  // the property `check-rank-injection` already pins for the UI's own table.
  //
  // THIS IS NOT A NEW HOME; IT IS THE ONLY ONE. `ui/interface.js` held these strings in
  // `_RANK_FACE` and `capabilities.js` grew a SECOND source in `_rankLabel` — a title-caser that
  // was correct for its one J07 caller (`uncategorized`, `guest`) and wrong the moment J36 gave it
  // a second: it rendered `vip` as **"Vip"** where the literal it replaced said **"VIP"**. Driven
  // rather than reasoned — the wrong label came out of the first run of the ten derived messages.
  //
  // SO AN ACRONYM IS THE REASON THIS IS A TABLE AND NOT A FUNCTION. No mechanical rule turns
  // `high-staff` into `High Staff` and `vip` into `VIP`; a caser needs a list of exceptions, and a
  // list of exceptions IS this table with the easy rows omitted. Writing every row makes the hard
  // one ordinary instead of special.
  //
  // COLOUR IS NOT HERE, deliberately. `_RANK_FACE` keeps it: a hex value is presentation with no
  // meaning to the reducer, the transport or a gate, and hoisting it would put paint in the module
  // that decides authority. A LABEL is different in kind — it is the rank's own name in prose, and
  // `capabilities.js` needs one to build a denial a person reads.
  const _LABELS = {
    "owner": "Owner", "high-staff": "High Staff", "staff": "Staff", "vip": "VIP",
    "player": "Player", "guest": "Guest", "uncategorized": "Uncategorized",
  };
  // TOTAL, and it falls back to the ladder's own name rather than to a blank or a throw: a rung
  // added without a label reads as its raw name — visibly unpolished, never invisible — which is
  // the same direction every other unknown in this module answers in.
  function labelOf(name) {
    const l = _LABELS[String(name)];
    return (typeof l === "string") ? l : String(name);
  }
  ...
  return {
    LADDER: LADDER.map((r) => ({ name: r.name, level: r.level, label: labelOf(r.name) })),
    NAMES: NAMES.slice(),
    TIER_COUNT,
    levelOf, nameOf, tierOf, atLeast, staggerMs, labelOf,
