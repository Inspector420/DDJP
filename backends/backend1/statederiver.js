...
const StateDeriver = (() => {
  ...
  // `enough` in a tier row of EITHER trust table — how many vouches or seals make that rank's voice
  // sufficient. One definition, named by both tables' entries below and read by `_rows`: the two
  // tables share a validator, so they share the number (J62).
  const TIER_ENOUGH = { min: 1, max: 50 };
  const SETTING_RANGES = {
    ...
    // ── THE LIST KINDS (J62) ─────────────────────────────────────────────────────────────────
    // Limits that used to be typed at their use sites AND restated in the panel, with nothing
    // comparing the copies. FLAT NUMBERS ONLY: `StreamManager.settingRanges()` copies top-level
    // fields and would hand a nested object to the panel by reference (the J07 trap). Nothing in
    // this file loops over these entries — each is read by name by its own validator.
    // `skipRoads`: how many roads a list may hold, and the bounds on each road's two counts.
    // ("Never both zero" is a rule about what a road MEANS, not a number, and stays in the validator.)
    skipRoads:            { minItems: 1, maxItems: 8, itemMin: 0, itemMax: 200 },
    // The two trust tables: one row per tier (the count is `Ranks.TIER_COUNT`, derived, never a
    // limit written here), each row's `enough` inside TIER_ENOUGH, or `null` for "never".
    vouchTable:           { enoughMin: TIER_ENOUGH.min, enoughMax: TIER_ENOUGH.max },
    checkpointTable:      { enoughMin: TIER_ENOUGH.min, enoughMax: TIER_ENOUGH.max },
  };
  ...
  // A LIST ENTRY and a TIER-TABLE ENTRY (J62). Neither carries `min`, `max`, `values` or `keys`, so
  // no older predicate can claim them first — the order below is safe for these two by shape.
  function _isListRange(r) {
    return !!r && typeof r.minItems === "number" && typeof r.maxItems === "number" &&
      typeof r.itemMin === "number" && typeof r.itemMax === "number";
  }
  function _isTierTable(r) { return !!r && typeof r.enoughMin === "number" && typeof r.enoughMax === "number"; }
  ...
  function settingKindOf(key) {
    ...
    if (_isListRange(r)) return "list";
    if (_isTierTable(r)) return "tiers";
    return null;
  }
  ...
  //
  // ── THE EVENT LIMITS, THE SECOND TABLE (J62) ─────────────────────────────────────────────────
  // Limits on what an EVENT may carry, as opposed to what a setting may hold. Kept apart from
  // SETTING_RANGES by owner ruling: that table is what the settings panel reads, and none of these
  // may ever be offered as a setting. Flat, one home each, read here and by the guard.
  const EVENT_LIMITS = {
    songIdMaxLength:  64,    // a YouTube id is 11; the headroom is for future id formats, not payloads
    songUrlMaxLength: 512,
    countMin:         0,     // `ddjp.count.set`'s `n` is an integer never below this
  };
  function eventLimits() { return Object.assign({}, EVENT_LIMITS); }   // a copy: the caller's, not ours
  const MAX_ID = EVENT_LIMITS.songIdMaxLength;
  const MAX_URL = EVENT_LIMITS.songUrlMaxLength;
  function songOf(c) {
  ...
  function _deriveFull(orderedEvents, seed) {
    ...
    for (const ev of list) {
      ...
      if (ev.type === "ddjp.dj.vote" || ev.type === "ddjp.dj.save") {
        ...
        const n = (typeof c.n === "number" && isFinite(c.n) && c.n >= EVENT_LIMITS.countMin && Math.floor(c.n) === c.n) ? c.n : null;
  ...
  function applySettingsEvent(base, s) {
    ...
    // Skip roads: accepted only as a COMPLETE, well-formed list — its length and each count inside
    // SETTING_RANGES.skipRoads — else the whole list is dropped rather than half-applied. `guestPlus`/`vipPlus` are
    // distinct-user thresholds; 0 means "this rank band is not required by this road".
    const SR = SETTING_RANGES.skipRoads;
    if (Array.isArray(s.skipRoads) && s.skipRoads.length >= SR.minItems && s.skipRoads.length <= SR.maxItems) {
      const roads = [];
      let ok = true;
      for (const r of s.skipRoads) {
        ...
        const gOk = Number.isSafeInteger(g) && g >= SR.itemMin && g <= SR.itemMax;
        const vOk = Number.isSafeInteger(v) && v >= SR.itemMin && v <= SR.itemMax;
    ...
    function _rows(v, withAlways) {
      ...
      for (const row of v) {
        ...
        const e = row.enough;
        const okEnough = (e === null) || (Number.isSafeInteger(e) && e >= TIER_ENOUGH.min && e <= TIER_ENOUGH.max);   // safe integers only
  ...
  return { derive, deriveAccepted, deriveRefusals, deriveBoth, buildSeed, defaultSettings, SETTING_RANGES, settingKindOf, projectHistory, canAdvance,
    ...
    applySettingsEvent, settingsBlobComplete, settingsClaimVerdict, voteEligible, HANDLED_TYPES: HANDLED_TYPES.slice(), eventLimits };
})();
