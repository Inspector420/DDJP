// ui/panels.js
// EVENT FEED · LOGS · WHO'S HERE · ROOM HISTORY — the read-only panels. Extracted from
// ui/interface.js at ddjp_387 as the second of the ten files roles.md §5 records.
//
// ── THE EVENT FEED ENDS AT renderFeedPanel, NOT AT THE NEXT BANNER ───────────────────────────
...
const Panels = (() => {
  ...
  function _feedRowText(row, nowTs) {
    const r = row || {};
    const who = r.sender ? H.shortName(r.sender) : "somebody";
  ...
  function renderFeedPanel() {
    ...
    refs.feedBox.appendChild(H.el("div", { class: "feed-head", text: lab.heading }));
    if (lab.empty) {
      refs.feedBox.appendChild(H.el("p", { class: "muted feed-empty", text: lab.empty }));
    }
    if (lab.reachNote) {
      refs.feedBox.appendChild(H.el("p", { class: "muted feed-reach", text: lab.reachNote }));
    }
    if (lab.truncated) {
      refs.feedBox.appendChild(H.el("p", { class: "muted feed-truncated", text: lab.truncated }));
    }
    const rows = fold.rows || [];
    if (rows.length) {
      QueuePanels._renderWindowedStack(refs.feedBox, () => rows, (r) => {
        const t = _feedRowText(r, now);
        const whoEl = H.el("span", { class: "who", text: t.who });
        whoEl.style.color = H.rankColor(H._rosterLevel(r.sender));
        // The same one trigger every surface uses (J14).
        if (r.sender) Roster._wireCardTrigger(whoEl, r.sender);
        return H.el("div", { class: "feed-row feed-" + (r.group || "other") }, [
          whoEl,
          H.el("span", { class: "feed-verb", text: t.verb }),
          H.el("span", { class: "feed-ago", text: t.ago }),
        ]);
      }, FEED_ROW_H);
    }
    if (lab.refusedNote) {
      refs.feedBox.appendChild(H.el("p", { class: "muted feed-refused", text: lab.refusedNote }));
    }
    refs.feedBox.appendChild(H.el("p", { class: "muted feed-sources", text: lab.sources }));
  }
  ...
  return {
    renderLogs, _logText, _fmtAgo, renderHistory, renderActivePanel, renderFeedPanel,
