...
const Shell = (() => {
  ...
  let rightTab = "chat"; // "chat" | "dm" | "people" | "roomset" | "gear"
  ...
  function _renderPaneNav() {
    ...
    for (const [id, label] of items) {
      ...
      b.dataset.pane = id;          // the notification dots find their switch by pane (ddjp_441)
      b.onclick = () => onPick(id);
      bar.appendChild(b);
    }
  ...
  function buildMainDom(main, room) {
    ...
    refs.joinBtn.onclick = () => {
      // The leave-lock only gates LEAVING (when active); Join is never behind a lock. The room's
      // join rule can still refuse it — `Roster.renderJoinBtn` greys the button and says why.
    ...
    // THE LIMIT, SHOWN BEFORE IT BITES (ddjp_430). A counter appears in the last eighth of the limit and
    // turns red over it; a send over it is refused by `Chat` with the text KEPT in the box. The count
    // is `Chat.charCount`, the same one the limit uses, so the number shown is the number that decides.
    refs.chatCount = el("span", { class: "chat-count", text: "" });
    const _updateChatCount = () => {
      let n = 0, max = 512;
      try { n = Chat.charCount(refs.chatInput.value.trim()); max = Chat.MAX_CHARS; } catch (e) {}
      refs.chatCount.textContent = n > max - max / 8 ? n + "/" + max : "";
      refs.chatCount.classList.toggle("over", n > max);
    };
    refs.chatInput.oninput = _updateChatCount;
    const sendChat = async () => {
      ...
      if (res && res.ok === false) {
        ...
        if (res.reason === "too-long") ChatPanels._setChatNote("Too long — " + res.count + "/" + res.max + " characters. Shorten it to send.");
      } else {
        ChatPanels._setChatNote("");
      }
    };
    ...
    refs.chat = el("div", { class: "chat" }, [
      ...
      refs.chatBox,
      refs.chatNote,
      el("div", { class: "chat-input-row" }, [refs.chatInput, refs.chatCount, el("button", { text: "Send", onclick: sendChat })])
    ]);

    // Tabs: chat · DMs · people · room-set · logs — one panel visible at a time. Each
    ...
    // THERE IS NO FEED TAB (owner ruling, ddjp_425). The event feed's rows sit between chat
    // messages in every tier now (`ui/chat.js` §CHAT); the fold behind them, `Room.recentEvents`,
    // is unchanged. See `main/04-features.md` §The event feed.
    ...
    const rightPanel = el("div", { class: "right-panel" }, [
      refs.rightBarMount,
      el("div", { class: "tabs" }, [refs.tabChat, refs.tabDM, refs.tabPeople, refs.tabRoomset, refs.tabGear]),
      refs.roster,
      refs.chat,
      refs.dm,
      refs.settings,
  ...
  UIBase.publish({
    ...
    // Is this pane ON SCREEN right now (ddjp_441)? Wide shows all three; Compact shows the player and one of
    // social/queues; Phone shows one. The notification dots ask this, so a dot means "not in front of you".
    paneShown: (p) => layoutMode === "wide" ? true
      : layoutMode === "compact" ? (p === "player" || compactSide === p)
      : phonePane === p,
