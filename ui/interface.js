...
const Interface = (() => {
  ...
  function showScreen(id) {
    ["screen-login", "screen-encryption", "screen-rooms", "screen-main"].forEach(s => {
  ...
  const STACK_VIEWPORT_H = 320;    // visible height (px)
  ...
  function _renderWindowedStack(parent, getList, rowFor) {
    const scroller = el("div", { class: "uq-scroll" });
    scroller.style.maxHeight = STACK_VIEWPORT_H + "px";
    ...
    function paint() {
      const list = getList() || [];
      const r = WindowedList.visibleRange(_stackScrollTop, STACK_VIEWPORT_H, STACK_ROW_H, list.length, STACK_BUFFER);
      topSpacer.style.height = r.topPad + "px";
      botSpacer.style.height = r.botPad + "px";
      clear(rowsBox);
      for (let i = r.start; i < r.end; i++) {
        const row = rowFor(list[i], i);
        row.style.height = STACK_ROW_H + "px";
  ...
  function renderMyQueue() {
    ...
    if (list.length === 0) {
      ...
      _renderWindowedStack(refs.queueBody, () => UserQueue.list(), (song, i) =>
        el("div", { class: "uq-item" }, [
  ...
  const CHAT_DOM_CAP = 400;        // most-recent messages kept in the DOM (text is cheap; raise freely)
  ...
  const CHAT_PAGE = 30;            // messages requested per scroll-up page

  // Per-box window state (ids currently shown + paging flags). Tied to the box
  // element so a rebuilt/cleared box starts clean automatically.
  function _chatState(box) {
    if (!box._chat) box._chat = { ids: new Set(), loading: false, done: false };
    return box._chat;
  }
  function _resetChatState(box) { if (box) box._chat = { ids: new Set(), loading: false, done: false }; }
  ...
  function _trimChat(box) {
    ...
    while (box.children.length > CHAT_DOM_CAP) {
      ...
      if (id) st.ids.delete(id);      // freed from the window -> reloadable on scroll-up
      box.removeChild(node);
    }
  }

  function _chatRow(id, sender, body) {
    ...
    if (id) msg.dataset.eid = id;    // window dedup key
    return msg;
  }

  function addChatMessage(id, sender, body) {
    const box = refs.chatBox || document.getElementById("chat-messages");
    if (!box) return;
    const st = _chatState(box);
    if (id) { if (st.ids.has(id)) return; st.ids.add(id); }   // window-owned dedup
    // Wire scroll handling once per box instance (the box is rebuilt on room
    // enter). At the bottom: trim. Near the top: page older history from Matrix.
    ...
    box.appendChild(_chatRow(id, sender, body));
    if (stick) {
      _trimChat(box);                // at the bottom: bound the DOM and follow the tail
      box.scrollTop = box.scrollHeight;
    }
    // Scrolled up: leave scroll + DOM untouched so the reader isn't disturbed; the
    // next return-to-bottom (scroll handler, or a later message while sticky) trims.
  }

  // Scroll-up: pull older plain messages from Matrix and prepend them, keeping the
  // view anchored (record height before, restore the delta after — content added
  // above must not shift what you're looking at). Guarded so only one page loads
  // at a time, and stops once the homeserver has no older history for this room.
  async function _loadOlderChat(box) {
    const st = _chatState(box);
    ...
    let res;
    try {
      res = await Chat.loadOlder(CHAT_PAGE, st.ids);
    } catch (e) {
      st.loading = false;
      return;
    }
    if (res && res.done) st.done = true;
    const older = (res && res.messages) || [];
    if (older.length) {
      const frag = document.createDocumentFragment();
      for (const m of older) {                       // oldest->newest; order preserved
        if (m.id) {
          if (st.ids.has(m.id)) continue;
          st.ids.add(m.id);
        }
        frag.appendChild(_chatRow(m.id, m.sender, m.body));
      }
  ...
  function showSaveNewKey({ recoveryKey, confirmMatch, onConfirm, onBack }) {
    ...
    copy.onclick = () => { try { navigator.clipboard.writeText(recoveryKey); copy.textContent = "Copied"; setTimeout(() => copy.textContent = "Copy", 1500); } catch (e) {} };
  ...
  return {
    ...
    showEnterRecoveryKey, showResetWarning, showSaveNewKey,
