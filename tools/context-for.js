...
// ── WHY THE DIRECTORY LIST IS READ FROM THE TREE AND NOT WRITTEN HERE (widened again) ───────
// The previous pattern hard-listed `app|core|ui|features|backends|tests|tools` AND required a
// slash, so it was blind in two directions at once:
//   · ROOT-LEVEL `.js` — `app.js`, `sw.js`, `sw-register.js` matched nothing. `app.js` is the
//     bootstrap, it CARRIES A `?v=` TAG, and four `Touches` fields name it. Every one of those
//     declarations was silently dropped. This is the SAME defect the `.html` widening above was
//     written to fix: that pass admitted root-level `.html` and stopped one case short.
//   · REAL DIRECTORIES MISSING FROM THE LIST — `journal/`, `lib/`, `src/` exist in the tree and
//     were not named, so a declaration naming one resolved to nothing.
// Both are the shape `08-build-and-deploy.md` warns about: a RESTATEMENT of a set drifting from
// the set it restates. So the set is no longer restated — it is read from the tree.
//
// ── AND WHY A BARE NAME IS NOT REPORTED AS ABSENT ───────────────────────────────────────────
// MEASURED before widening: admitting every backticked `*.js` matched 57 new tokens in the
// roadmap, of which FIVE exist. The other 52 are ordinary prose — entries say `room.js`,
// `player.js`, `vouch.js` without a directory all the time. Reporting those as "named but absent"
// would have buried the four real broken declarations under 52 false ones, which is worse than
// the blindness it replaced: a warning that is usually wrong gets ignored, including the time it
// is right.
//
// So the two shapes are judged differently, and the rule is about what the WRITER declared:
//   · a PREFIXED path (`ui/interface.js`) is a declaration about the tree. Missing => REPORT.
//   · a BARE name (`room.js`) is prose unless it resolves at the root. Missing => IGNORE.
// `app.js` is admitted by the second rule because it EXISTS, which is the only evidence available
// and the same evidence `check-guard-subject` uses: a path is admissible if it resolves.
const TREE_DIRS = fs.readdirSync(ROOT, { withFileTypes: true })
  .filter((d) => d.isDirectory() && d.name !== "node_modules" && !d.name.startsWith("."))
  .map((d) => d.name).sort();
const SRC_RE = new RegExp(
  "`((?:(?:" + TREE_DIRS.join("|") + ")\\/[\\w./-]+\\.(?:js|html)|[\\w.-]+\\.(?:js|html)))`", "g");
...
function main() {
  ...
  // A BARE name that does not resolve is prose, not a broken declaration — see SRC_RE's note.
  // Only a path the writer gave a directory to is a claim about the tree, and only a claim can
  // be wrong.
  const missing = files.filter((f) => !fs.existsSync(path.join(ROOT, f)) && f.includes("/"));

  // ── SELF-MENTION: USUALLY EXPLANATORY, OCCASIONALLY REAL ──────────────────────────────────
  // J33 declared `backends/backend1/matrixbridge.js` and `app.js`, then added a sentence inside
  // the same paragraph explaining that the field existed *so `tools/context-for.js` can read it*.
  // This reads the WHOLE paragraph for paths, so that remark put this file into the job's scope —
  // a file J33 does not touch. A session scoping J33 got three files where two were real.
  //
  // It is a WARNING rather than a rule because the opposite case exists and is legitimate: J55 is
  // the subject-map job and genuinely touches this tool, which states the map's coverage on every
  // run. Nothing can mechanically tell a declaration from a remark about the declaration, so the
  // judgement is handed to the reader at the moment they would otherwise be misled.
  const selfMention = real.some((f) => f === "tools/context-for.js");
  ...
  if (selfMention) {
    out.push("  ? this scope includes THIS TOOL (tools/context-for.js). That is usually an");
    out.push("    explanatory remark inside the **Touches** paragraph rather than a file the job");
    out.push("    touches — the paragraph is read whole, so a sentence about the field lands in");
    out.push("    the scope. Check the entry before reading it. Some jobs DO touch this tool.");
  }
...
// ── REQUIRABLE WITHOUT RUNNING, SO A GUARD CAN TEST THE REAL MATCHER ────────────────────────
// `check-scope-matcher` asserts that every file carrying a `?v=` tag is matchable here. It must
// test THIS pattern, not a copy of it — a guard holding its own regex would agree with a drifted
// one forever, which is the restatement failure this file was just widened to fix.
if (require.main === module) main();
module.exports = { SRC_RE, TREE_DIRS };
