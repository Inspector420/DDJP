...
const EXEMPTIONS = {
  // RE-POINTED AT ddjp_392. This exemption is keyed by FILE PATH, so the ui/ split broke it the
  // moment `_resetChatState` moved to ui/chat.js — the waiver went on pointing at a file that no
  // longer holds the binding, and `no-unused-vars` fired on a decision this tree has already made
  // twice. Same family as a stale `// SUBJECT:` line: an exemption that names a location is a
  // claim about where something lives, and a split is exactly when that claim goes stale.
  "no-unused-vars:ui/chat.js:_resetChatState":
...
function build() {
  ...
  return {
    ...
    overrides: [{
      // The `_resetChatState` waiver below is scoped to the file that HOLDS the binding, and that
      // file changed at ddjp_392. Both are listed because the split is in progress: the block
      // travels with the declaration, not with the original filename.
      files: ["ui/chat.js"],   // ui/interface.js ceased to exist at ddjp_397
