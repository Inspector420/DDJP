...
const EXEMPTIONS = {
  // THE PARKED `_resetChatState` FINDING IS GONE (ddjp_425), and so is its file-scoped override. It
  // waived `no-unused-vars` for one binding kept only because two harnesses brace-matched from it;
  // chat was rebuilt model-first, the binding and both extractions went with the shape it served, and
  // an exemption naming a binding that no longer exists is a stale claim about where something lives
  // (the fifty-second signature). Deleted rather than left, because a waiver that matches nothing today
  // is one a future binding of that name would inherit without anybody deciding it.
