...
// The marker paths and the rule for reading them live in `mutate-residue.js`, because the guard that
// checks for a sweep's leftovers needs the same answer and must never load THIS file (it sweeps on load).
const Residue = require("./mutate-residue.js");
const { inflight: INFLIGHT, bak: INFLIGHT_BAK, pid: PIDFILE } = Residue.paths();
...
//
// ── AND A MARKER WHOSE OWNER IS STILL RUNNING IS NOT A CRASH ────────────────────────────────
// This used to restore whenever an inflight record existed, so starting a second sweep while one
// ran would put back the file the first had just mutated — the two-sweeps-erase-each-other failure
// `09-roadmap.md` §8 describes, performed by this function. A live owner now REFUSES the start.
// And a run killed BETWEEN trials leaves only its pid file; that used to survive every recovery and
// ship in a package. Observed while building `ddjp_415`, when a harness reaped a backgrounded sweep mid-trial:
// the file stayed mutated, the pid stayed behind, and `run-all` had no way to see either —
// `tests/check-sweep-residue.js` exists for that half.
function recoverIfCrashed() {
  const r = Residue.residue(process.pid);
  if (r.state === "clean") return;
  if (r.state === "other-sweep") {
    console.log("REFUSED — a live sweep (pid " + r.pid + ") owns this tree. Two sweeps at once erase " +
                "each other's mutations; wait for it, or stop it and re-run.");
    process.exit(1);
  }
  if (r.state === "crashed") {
    const rec = r.rec;
    if (!rec || typeof rec.rel !== "string") {
      throw new Error("inflight record unreadable — restore " + INFLIGHT_BAK + " by hand; it holds the original");
    }
  ...
  if (fs.existsSync(PIDFILE)) {
    ...
    console.log("CLEARED — a stale pid file from a run that died" +
                (r.state === "stale-pid" ? " between trials (no file was mutated)." : "."));
  }
}
