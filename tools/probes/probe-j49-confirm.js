...
//
// That is a claim about `ui/player.js`, not about `playback.js`. R1 is therefore the row that
...
line("");
line("=== R1 — THE PROVIDER CONTRACT, driven from ui/player.js's own source ===");
...
if (!ps.ok) {
  ...
  // The list lives in the harness since ddjp_415, so `check-duration-provider` drives the SAME
  // shapes this runner reports on — one list, two readers.
  ...
  if (falsyIdShapes === 0) {
    ...
    line("  because of ui/player.js rather than because of anything in playback.js.");
  }
}
