...
const StateDeriver = (() => {
  ...
  // so a default only ever lives in one place. Pure — safe under check-purity.
  function defaultSettings() {
    return { chat: "uncategorized", vis: "private", bg: null };
  }
  ...
  function derive(orderedEvents) {
    ...
    // Room PLAY HISTORY: the ordered list of songs that actually played, derived
    // as a byproduct of the SAME fold that produces nowPlaying — so it can never
    // disagree with what really played (14: Room History is a shared, derived view
    // of the play log, not a separate stored list). A play/skip's videoId is NOT
    // in its event body; it's whatever the reducer pops from the head DJ's buffer
    // here, which is exactly why this must live inside derive() and not scan events
    // in isolation. Each entry: { videoId, dj, at (ev.ts), pi, skipped }. Pure:
    // `at` is the event's own ts (already used for startedAt), no clock/storage.
    // Bounded to HISTORY_MAX retained entries (oldest dropped) so a long replayed
    // log can't grow the derived array without limit — it stays a recent window,
    // recomputed each ingest like the rest of the derived state.
    const history = [];
    ...
    for (const ev of list) {
      ...
      if (ev.type === "ddjp.dj.join") {
        ...
        if (idx < 0) continue;                        // not in my buffer -> no-op
    ...
    return { nowPlaying, rotation, settings, history };
  }
  ...
  return { derive, defaultSettings, projectHistory };
})();
