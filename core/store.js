...
// not be referenced anywhere but the storage layer. (Spec: 09 §1; docs/main/06-storage.md
// "Storage boundary".)
//
// ENGINE PER DOMAIN — the facade's whole point is that this table can change without
// any caller changing. As shipped:
//   StorageIO (sync localStorage): config · prefs · stagger   — small, must never grow
//   IndexedDB (async, bulk):       background · images · meta · playlists
//   IDB with a StorageIO fallback: logs · queue                — degrade to RAM/sync if IDB is absent
// So do NOT assume a domain is synchronous: only the first row is. `meta` additionally
// carries a synchronous RAM mirror (`peek`) because display resolvers cannot await —
// `peek` returning null means "not loaded this session", never "no such value".
// The original increment moved domains here one at a time WITHOUT changing this public surface or
...
const Store = (() => {
  ...
  // (`uq_<spaceId>`) to IndexedDB behind these methods — the deliberate docs/main/06-storage.md
  ...
  // --- history (per-room played-song table — IndexedDB, derived-and-banked) ---
  // The pane's rows, kept so a reload does not re-fold them and — the actual point — so the RAW
  // EVENTS behind a settled row can be dropped. Without this the table is recomputed from the
  // events every load, which means the events can never go, which means the 5000 cap saves
  // nothing.
  //
  // CLASSIFIED CACHE, NOT TRUTH, and that is what makes it safe to store. Every row is derivable
  // from events plus a checkpoint; losing this file costs one re-fold, never a fact. So it is
  // written through without ceremony and read back best-effort — a corrupt or absent table is an
  // empty one, and the rebuild path fills it.
  //
  // NO METADATA HERE. Title, duration and thumbnail live in `meta` and `images` above, user-global
  // and LRU-capped, because a video's title does not change between rooms. `playlistdoc.js` states
  // the law this follows: a track is its videoId and nothing else. A row carries only what the
  // stream cannot regenerate — which song, when, who, skipped, the proving event, and the floor it
  // was settled under.
  //
  // BOUNDED BY THE CALLER, deliberately. `History.MAX` is the cap and it already evicts oldest-
  // first; a second cap here would be a second answer to one question, and the two would drift.
  const history = {
    load(spaceId) {
      if (!spaceId || !_idbOk()) return Promise.resolve(null);
      return IDB.get("kv", IDB.keyFor({ room: spaceId }, "history", "rows"))
        .then((v) => (Array.isArray(v) ? v : null))
        .catch(() => null);
    },
    persist(spaceId, rows) {
      if (!spaceId || !_idbOk() || !Array.isArray(rows)) return;
      IDB.set("kv", IDB.keyFor({ room: spaceId }, "history", "rows"), rows).catch(() => {});
    },
    clear(spaceId) {
      if (!spaceId || !_idbOk()) return;
      IDB.del("kv", IDB.keyFor({ room: spaceId }, "history", "rows")).catch(() => {});
    },
  };
  ...
  const images = {
    ...
    // Snapshot { videoId: sizeBytes } of every cached thumbnail — ONE kv read of the
    // LRU index, no blob loads. Powers the export size estimate (15): which selected
    // songs have art and how big, computed instantly as the user toggles the checklist.
    sizes() {
      if (!_idbOk()) return Promise.resolve({});
      return _imgLoadIndex().then((ix) => {
        const out = {};
        for (const k in ix) if (ix[k] && typeof ix[k].size === "number") out[k] = ix[k].size;
        return out;
      }).catch(() => ({}));
    },
    _touch(id) {
  ...
  // BOUNDED to META_CACHE_CAP entries, LRU-evicted through the same pure
  // indexUpsertEvict/evictionPlan the thumbnail cache uses, so it can never grow
  // without limit. Every entry is regenerable from the videoId, so eviction is
  // always safe — it costs at most one re-fetch. A tiny index in `kv` holds
  // {order} per id; order is a session counter seeded above the persisted max, so
  // LRU stays monotonic across reloads without touching a clock.
  // A synchronous RAM MIRROR of what this session has already loaded or written.
  // IDB is async, but display resolvers (MediaLength.displayDuration) are sync, so a
  // sync read path is required or the cache tier can never be consulted at all. This
  // is read-through, never authoritative: `peek` returns null until something has
  // loaded that id, and MetadataService.get loads it on the way to rendering a row.
  const META_CACHE_CAP = 2000;   // tune freely; a row is a title + a few numbers
  const _metaRam = Object.create(null);   // null-proto: videoId is attacker-controlled
  let _metaSeq = 0, _metaSeqReady = false;
  function _metaIndexKey() { return IDB.keyFor(null, "meta", "_index"); }
  function _metaRecKey(videoId) { return IDB.keyFor(null, "meta", String(videoId)); }
  function _metaLoadIndex() {
    return IDB.get("kv", _metaIndexKey()).then((v) => (v && typeof v === "object") ? v : {}).catch(() => ({}));
  }
  function _metaSeedSeq(ix) {                // continue the LRU counter above prior sessions
    if (_metaSeqReady) return;
    let mx = 0; for (const k in ix) if (ix[k] && ix[k].order > mx) mx = ix[k].order;
    if (mx > _metaSeq) _metaSeq = mx;
    _metaSeqReady = true;
  }
  const meta = {
    // SYNC read of the RAM mirror — a hint for sync display resolvers, never truth.
    // null means "not loaded this session", NOT "no such metadata".
    peek(videoId) {
      if (!videoId) return null;
      const v = _metaRam[String(videoId)];
      return (v && typeof v === "object") ? v : null;
    },
    load(videoId) {
      if (!videoId || !_idbOk()) return Promise.resolve(null);
      const id = String(videoId);
      return IDB.get("kv", _metaRecKey(id))
        .then((v) => { if (v && typeof v === "object") _metaRam[id] = v; return v || null; })
        .catch(() => null);
    },
    persist(videoId, rec) {
      ...
      const id = String(videoId);
      _metaRam[id] = rec;
      return IDB.set("kv", _metaRecKey(id), rec)
        .then(() => _metaLoadIndex())
        .then((ix) => {
          _metaSeedSeq(ix);
          const r = IDB.indexUpsertEvict(ix, id, { order: ++_metaSeq }, META_CACHE_CAP);
          for (const k of r.evicted) delete _metaRam[k];      // mirror follows the cap
          return Promise.all(r.evicted.map((k) => IDB.del("kv", _metaRecKey(k))))
            .then(() => IDB.set("kv", _metaIndexKey(), r.index));
        })
        .then(() => true).catch(() => false);
    },
    clear(videoId) {
      ...
      const id = String(videoId);
      delete _metaRam[id];
      return IDB.del("kv", _metaRecKey(id))
        .then(() => _metaLoadIndex())
        .then((ix) => { if (!(id in ix)) return null; delete ix[id]; return IDB.set("kv", _metaIndexKey(), ix); })
        .catch(() => {});
    },
  };
  ...
  // --- stagger offset (DEVICE-LOCAL) ---------------------------------------
  // How long THIS client waits inside the owner slot before acting. Device-local, not a
  // room setting: it exists so an owner and their bot — both at owner authority, both
  // firing at slot 0 — do not collide. The bot stays at 0 and the human sets a value.
  // Ignored for every other rank, whose spread comes from the shared jitter window.
  // Clamped to one step by Ranks.staggerMs, so an owner can at worst tie with high-staff.
  const stagger = {
    offsetMs() {
      try {
        const v = StorageIO.load("staggerOffsetMs");
        return (typeof v === "number" && isFinite(v) && v >= 0) ? v : 0;
      } catch (e) { return 0; }
    },
    setOffsetMs(ms) {
      const v = (typeof ms === "number" && isFinite(ms) && ms >= 0) ? Math.round(ms) : 0;
      try { StorageIO.save("staggerOffsetMs", v); } catch (e) {}
      return v;
    },
  };
  ...
  return { config, logs, queue, history, background, images, meta, playlists, durability, account, prefs, stagger };
})();
