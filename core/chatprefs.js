...
const ChatPrefs = (() => {
  ...
  // The mention chime's volume, a percent (ddjp_431). Default a third — owner ruling: loud enough to hear,
  // quiet enough to be safe on headphones.
  const SOUND_RANGE = { min: 0, max: 100, dflt: 33 };
  const DIM_RANGES = {
  ...
  let _state = null;
  // WHOSE state `_state` is (ddjp_439). It was loaded once and kept, so a second account without a page reload
  // saw the first one's conversations — reproduced. `_st()` reloads when the storage namespace has moved.
  let _stateNs = null;
  const _nsNow = () => { try { return (typeof StorageIO !== "undefined" && StorageIO.currentNamespace) ? StorageIO.currentNamespace() : ""; } catch (e) { return ""; } };
  const _listeners = [];
  ...
  function _defaults() {
    return {
      ...
      botView: false,
      // The mention chime (ddjp_431): ON by default at a third of full volume (owner ruling), this device only.
      mentionSound: true,
      mentionVolume: SOUND_RANGE.dflt,
      // The DM chime (ddjp_432): its OWN switch and volume (owner ruling), defaulting as the mention one does.
      dmSound: true,
      dmVolume: SOUND_RANGE.dflt,
      ...
      feedShow: {},      // feed kind (event type) -> true|false: a choice THIS DEVICE made. See below.
    };
  }

  // ── WHICH FEED KINDS SHOW IN CHAT — A DEVICE CHOICE, ONE PER KIND (owner rulings ddjp_425, ddjp_427) ─
  // The event feed's rows sit between chat messages. Which kinds of room event appear there is chosen
  // per device, ONE CHECKBOX PER KIND (upvotes apart from saves, song starts apart from skips), beside
  // the dim sliders. That is only allowed because NOTHING ACTS ON what the feed displays: it narrates
  // the log and is nobody's basis for an action, which is the test `roles.md` §6 sets (*a PREFERENCE /
  // the BASIS FOR AN ACTION*). It is not a room setting — a key in `defaultSettings()` would move every
  // checkpoint fingerprint — and no backend module reads this one, so nothing here can reach a seed.
  //
  // THE DEFAULT IS NOT HELD HERE. Each kind's default is `Room.FEED_KINDS[type].shownByDefault`,
  // beside its name, and the caller hands it in — core may not depend upward on features, and a
  // second list of kinds here would be a second copy of the feature's vocabulary to drift.
  //
  // ONLY A CHANGE FROM THE DEFAULT IS STORED. `setFeedShown` forgets the entry when a person picks
  // the default again, so a later change to a default reaches every device that never chose
  // otherwise. `load()` keeps only string keys with BOOLEAN values, so the field cannot carry
  // anything but kind names and one bit. `feedOff` — the per-BUNDLE map of ddjp_425, keyed by group
  // names no kind has — is dropped on load rather than read. Capped: this is the synchronous
  // localStorage tier, which must never grow.
  const FEED_SHOW_CAP = 16;
  ...
  function tierFold(list, row, cap) {
    ...
    for (const r of (Array.isArray(list) ? list : [])) {
      ...
      out.push({ tier: String(r.tier), lastTs: Number(r.lastTs) || 0, readTs: Number(r.readTs) || 0, mentionTs: Number(r.mentionTs) || 0 });
    }
    out.push({ tier: String(row.tier), lastTs: Number(row.lastTs) || 0, readTs: Number(row.readTs) || 0, mentionTs: Number(row.mentionTs) || 0 });
  ...
  function load() {
    ...
    _state.mentionVolume = _clampToRange(_state.mentionVolume, SOUND_RANGE);
    _state.mentionSound = (typeof _state.mentionSound === "boolean") ? _state.mentionSound : true;
    _state.dmVolume = _clampToRange(_state.dmVolume, SOUND_RANGE);
    _state.dmSound = (typeof _state.dmSound === "boolean") ? _state.dmSound : true;
    ...
    _state.tiers = (Array.isArray(_state.tiers) ? _state.tiers : [])
      ...
      .map((r) => ({ tier: String(r.tier), lastTs: Number(r.lastTs) || 0, readTs: Number(r.readTs) || 0, mentionTs: Number(r.mentionTs) || 0 }))
    ...
    // Rebuilt, never trusted as loaded — the same rule the DM index and tier markers follow.
    const fs = (_state.feedShow && typeof _state.feedShow === "object" && !Array.isArray(_state.feedShow)) ? _state.feedShow : {};
    _state.feedShow = {};
    for (const k of Object.keys(fs).slice(0, FEED_SHOW_CAP)) {
      if (k && typeof fs[k] === "boolean") _state.feedShow[String(k)] = fs[k];
    }
    delete _state.feedOff;   // the per-bundle map of ddjp_425; its keys were group names, never kinds
    return _state;
  }
  function _st() { return (_state && _stateNs === _nsNow()) ? _state : load(); }
  ...
  function dmClear() { _st().dms = []; _saveIfMoved(); }

  // --- per-tier chat views and read markers (J12) ---
  // ── READ MARKERS ARE NOT DISPLAY PREFERENCES (ddjp_442) ──────────────────────────────────────────
  // `onChange` means "how chat DISPLAYS changed" (images, links, hosts, feed kinds, sounds, layout), and its one
  // listener rebuilds every chat row and redraws the settings panel — right for those. The read markers used to
  // fire it too, and always, even when nothing moved; drawing the notification dots marks the on-screen tier
  // read, the listener redrew the tier strip, the strip drew the dots — and one message fired it 3141 times,
  // each a full chat rebuild, until the browser gave up (reproduced; `check-chat-view` PART N). So a marker
  // SAVES — only when the stored state actually moved — and never fires `onChange`. Whoever moves a marker
  // redraws what it affects (the tier strip, the DM list, the dots), as they already did.
  let _savedMarkers = null;
  function _saveIfMoved() {
    const now = JSON.stringify({ t: _st().tiers, d: _st().dms });
    if (now === _savedMarkers) return;
    _savedMarkers = now;
    _save();
  }
  function tierList() { return _st().tiers.map((r) => ({ tier: r.tier, lastTs: r.lastTs, readTs: r.readTs, mentionTs: r.mentionTs || 0 })); }
  function tierTouch(tier, lastTs) {
    ...
    s.tiers = tierFold(s.tiers, {
      ...
      lastTs: Math.max(ts, (prev && prev.lastTs) || 0),
      readTs: (prev && prev.readTs) || 0,
      mentionTs: (prev && prev.mentionTs) || 0,
    });
    _saveIfMoved();
    return tierList();
  }
  ...
  // ── MENTIONS (ddjp_431) — ONE MORE SCALAR PER TIER, NEVER A MESSAGE ───────────────────────────────
  // `mentionTs` is the stamp of the newest message in that tier that mentioned this account. The purple
  // dot is `mentionTs > readTs`, so reading the tier clears it through the same forward-only marker that
  // clears the yellow one — one read state, two questions about it. A scalar, like the rest of the row:
  // nothing here can hold who said it or what they said.
  function tierMention(tier, ts) {
    if (!tier) return tierList();
    const s = _st();
    let row = s.tiers.find((r) => r.tier === tier);
    if (!row) { tierTouch(tier, ts); row = s.tiers.find((r) => r.tier === tier); if (!row) return tierList(); }
    row.mentionTs = Math.max(Number(row.mentionTs) || 0, Number(ts) || 0);
    _saveIfMoved();
    return tierList();
  }
  function tierMentioned(tier) {
    const row = _st().tiers.find((r) => r.tier === tier);
    return !!(row && (row.mentionTs || 0) > row.readTs);
  }
  function tierUnreadCount() { return _st().tiers.filter((r) => r.lastTs > r.readTs).length; }
  function tierClear() { _st().tiers = []; _saveIfMoved(); }
  ...
  function mentionSound()  { return _st().mentionSound !== false; }
  function mentionVolume() { return _clampToRange(_st().mentionVolume, SOUND_RANGE); }
  function setMentionSound(v)  { _st().mentionSound = !!v; _saveIfMoved(); }
  function setMentionVolume(v) { _st().mentionVolume = _clampToRange(v, SOUND_RANGE); _saveIfMoved(); }
  function dmSound()  { return _st().dmSound !== false; }
  function dmVolume() { return _clampToRange(_st().dmVolume, SOUND_RANGE); }
  function setDMSound(v)  { _st().dmSound = !!v; _saveIfMoved(); }
  function setDMVolume(v) { _st().dmVolume = _clampToRange(v, SOUND_RANGE); _saveIfMoved(); }
  function setChatTier(t) { _st().chatTier = (typeof t === "string" && t) ? t : null; _saveIfMoved(); }

  // Feed kinds in chat (ddjp_425, per kind since ddjp_427). `kind` is an event type, and `dflt` is
  // that kind's `shownByDefault`, handed in by the caller. A kind with no stored choice follows its
  // default; with no default handed in it is SHOWN, because hiding what nobody chose to hide would be
  // a filter nobody set.
  function feedShown(kind, dflt) {
    const v = _st().feedShow[String(kind)];
    return (typeof v === "boolean") ? v : (dflt !== false);
  }
  function setFeedShown(kind, on, dflt) {
    if (typeof kind !== "string" || !kind) return;
    const m = _st().feedShow;
    const want = !!on;
    if (want === (dflt !== false)) delete m[kind];                         // the default again: forget it
    else if (kind in m || Object.keys(m).length < FEED_SHOW_CAP) m[kind] = want;
    _save(); _emit();
  }
  ...
  return {
    ...
    tierMention, tierMentioned, mentionSound, mentionVolume, setMentionSound, setMentionVolume, SOUND_RANGE,
    dmSound, dmVolume, setDMSound, setDMVolume,
    chatTier, setChatTier,
    feedShown, setFeedShown, FEED_SHOW_CAP,
