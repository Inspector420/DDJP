...
const Backends = (() => {
  // The slots an engine must fill to be a backend. This list is a GRAMMAR, not a population: every
  // doc in the tree named one fewer until `ddjp_399`, because `MatrixAccount` joined the interface
  // at J59 in the same change as the file and no list was widened with it. A seam built to the old
  // list is one the account half cannot pass through — so the count lives here and nowhere else.
  ...
  function register(modeId, impl, opts) {
    ...
    // is built from far fewer rooms than a decentralised one, because the rank ladder stops being
    // encoded in the channel topology. Since both engines ride the same transport module,
    ...
    // SETTINGS THIS ENGINE NEVER ACTS ON. Declared by the engine beside its label, so the settings
    // panel can leave them out of a room this engine runs rather than offer controls that change
    // nothing there — the owner's rule for the panel, and the engine is the one place that knows.
    // Display only: the keys stay in the settings blob and the fold exactly as before.
    if (Array.isArray(o.idleSettings)) entry.idleSettings = o.idleSettings.slice();
    ...
    // PEER BACKUPS. An engine whose rooms never rebuild from peers' backups says so, and nothing then
    // attaches or sends them (the owner's ruling, ddjp_423). Absent means yes: backend1 is the default.
    if (o.backups === false) entry.backups = false;
    return entry;
  }
  ...
  function describe(modeId) {
    ...
    return { id: e.modeId, label: e.label, blurb: e.blurb || "",
             idleSettings: (e.idleSettings || []).slice(), backups: e.backups !== false };
  }
