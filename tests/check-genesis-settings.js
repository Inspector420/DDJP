...
// SUBJECT: features/room.js, backends/backend1/settingsproof.js, backends/backend1/streammanager.js
// `streammanager.js` ADDED at `ddjp_384` — under-declared. PART A judges the settings DOOR's own
// behaviour twice: `defaultSettings()` must return a FRESH object each call (a shared one lets any
// caller rewrite every room's idea of default), and the door must PASS THROUGH the reducer's defaults
// rather than restate them — an assertion that exists precisely because a door answering from its own
// literal would pass everything else while drifting. Both go false only if `streammanager.js`
// changes. Same shape and same module as the `blockedReasons()` fresh-copy row corrected in
// `check-blocked-reason`.
// `statederiver.js` is NOT added: it is the STANDARD the door is measured against. Because the door
// passes through, a new dial moves both sides together and the agreement assertion cannot fail —
// the delegation discriminator, as in `check-actions`.
// `features/room.js` STAYS despite never being loaded: PART C reads it as SOURCE and asserts on its
// create path, which the header explains (it cannot be loaded headlessly). Not-loaded is not
// not-judged.
//
