...
// ── THE SUBJECT LINE WAS WRONG, AND IT WAS THE DEFECT — not the assertions ────────────────────
// This guard was scoped at ddjp_386 as having no non-vacuity assertion and passing on an empty
// match. Driven, that is false: PART 5 asserts `if (!w.ok) fail(...)` on every reading of the
// ENDED branch and goes RED when the extraction finds nothing. What was actually wrong is this
// line, which named `features/playback.js` alone while half of PART 5's subject is the shipped
// UI's own ENDED branch, extracted and executed. That half lives in `ui/player.js` since the
// split and is now declared — a subject line is an instruction about where to look, and a
// half-true one is a stale instruction, which this tree holds to be worse than a stale fact.
...
// `StateDeriver`'s seed path, the signal out of `ui/player.js`'s own ENDED branch, executed.
//
