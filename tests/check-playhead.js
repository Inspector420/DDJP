...
// SUBJECT: features/serverclock.js, features/playback.js, ui/shell.js, ui/player.js, features/medialength.js, features/mediablocked.js, features/skip.js, features/reactions.js
//
...
// 4,800-line file cannot stand in for it.
{
  ...
  ok(/elapsedSec:\s*_elapsedSec/.test(pb),
    "B: Playback exposes its server-time elapsed, which the loader asks first "
    + "— the UI renders, it does not compute. The feature that already had the answer "
    + "simply was not asked");

  // `_doLoad` and the load call site both moved to ui/player.js (roles.md §5).
  const ui = fs.readFileSync(path.join(__dirname, "..", "ui/player.js"), "utf8");
...
{
  const offenders = [];
  // WIDENED rather than swapped: the property is that NO comparison of a server stamp uses a
  // raw local clock, and it holds of both halves of the split file. Dropping one would shrink
  // the population silently, which is the loss this split was measured for.
  const files = ["features/playback.js", "ui/base.js", "ui/shell.js", "ui/player.js", "features/medialength.js",
