...
// PART B extracts `_settingDisplayName`, which moved to ui/chat.js with the CHAT banner at
// ddjp_392 — the settings panel names a chat tier, so the label function lives with chat while
// the panel stays here. Both are read: the property spans them, and re-pointing at either alone
// would make one half look absent.
const src = ["ui/base.js", "ui/shell.js", "ui/chat.js", "ui/settings.js", "ui/screens.js"]
...
function node(tag) {
  return { tag, kids: [], cls: null, text: null, value: "", attrs: {},
...
// ═══ PART A — THE DELEGATION SECTION RUNS, IN BOTH BRANCHES ═════════════════════════════════
{
  ...
  function run(opts) {
    ...
    const ctx = {
      ...
      // `_renderChatTierStrip` moved to ui/chat.js at ddjp_392 and is a repaint this part does not
      // judge. `_settingDisplayName` is NOT stubbed here — see below: it is EXTRACTED and the real
      // function is bound onto `ChatPanels` inside the context. A stub returning the key passed
      // the reference and failed the assertion, which is this guard refusing a harness that lies.
      ChatPanels: { _renderChatTierStrip: () => {} },
      // The settings panels moved to ui/settings.js at ddjp_393 and reach their primitives
      // through the host seam, so the extracted section builder calls `H.el(...)`. Built from
      // the SAME `el` passed above — one definition of what an element is, not two.
      H: { el, clear: (n) => { n.children.length = 0; },
           settingsLocked: () => false, setSettingsLocked: () => {},
           prefsLocked: () => false, setPrefsLocked: () => {},
           renderExportSection: () => {} },
      _delegationOpen: !!o.open,
    };
    vm.createContext(ctx);
    // THE REWORK'S LAYOUT PIECES, the real ones where they are declarations: `_rowHead` and the
    // delegation grouping come out of ui/settings.js; `_box`/`_name`/`_rankUp` are one-line
    // plumbing (where to append, the name table, a rank's label) and are not what this part tests.
    const grpAt = src.indexOf("const _DELEG_GROUPS = [");
    const grpSrc = grpAt < 0 ? "" : src.slice(grpAt, src.indexOf("];", grpAt) + 2);
    const layout = (extract("_rowHead") || "") + "\n" + grpSrc +
      "\nconst _box = () => refs.settingsBox; const _name = (k) => _settingDisplayName(k); const _rankUp = (n) => n;\n";
    vm.runInContext(nameFn + "\n" + layout + fn + ";globalThis.__f = _renderDelegationSetting;" +
      "\n;globalThis.__name = _settingDisplayName;" +
      // THE REAL EXTRACTED FUNCTION, BOUND ONTO THE GLOBAL THE MOVED CODE NOW CALLS. Since
      // ddjp_392 the settings row calls `ChatPanels._settingDisplayName(...)` rather than the bare
      // name, so the reference has to resolve to the SAME function this part extracted — not to a
      // stub. A stub returning the key satisfied the reference and failed the label assertion,
      // which is the harness being refused rather than the subject.
      "\n;globalThis.ChatPanels = Object.assign(globalThis.ChatPanels || {}," +
      " { _settingDisplayName: _settingDisplayName });", ctx);
    let threw = null;
  ...
  ok(/Nothing is shared/.test(cText),
  ...
  ok(/1 setting is shared: /.test(wText),
...
// and nothing between `renderSettings()` and the end of `enterMainScreen` is inside a `try`.
{
  ...
  // THE 49TH SIGNATURE. The panels moved to ui/settings.js at ddjp_393, so a repaint written
  // inside that file is still `renderSettings();` while one written from ui/interface.js is now
  // `Settings.renderSettings();`. The property — that the commit repaints — did not change; the
  // grammar did. Whichever form is present, the LAST one is the call this part is about.
  const callAt = Math.max(body.lastIndexOf("\n    renderSettings();"),
...
// what the first wrote.
{
  ...
  function drive(wanted) {
    ...
    const ctx = {
      ...
      // The right panel moved to ui/queue.js at ddjp_388, so the reader calls it through
      // `QueuePanels`. Same recorder, not a second one — two stubs for one panel are free to
      // disagree about what "the tab was repainted" means.
      QueuePanels: { renderRightPanel: () => tabs.push(ctx.rightTab) },
      // The saves reader moved to ui/screens.js at ddjp_394 and `rightTab` is owned by
      // ui/interface.js, so the switch goes through the published setter now. The recorder writes
      // the SAME `ctx.rightTab` the assertion reads — the destination is still observed, only the
      // route to it changed.
      // The block below sits inside a try/catch, so a MISSING stub is swallowed and shows up as
      // "the scroll never happened" rather than as an error — the stub list has to be complete,
      // not merely enough to resolve the first reference.
      H: { setRightTab: (v) => { ctx.rightTab = v; }, rightTab: () => ctx.rightTab,
           _relockAllPanels: () => {} },
      // The repaint the destination triggers also moved: `renderSettings` is ui/settings.js's now.
      Settings: { renderSettings: () => {} },
      // The settings panel repaints the chat tier strip when a tier setting changes, and that
      // moved to ui/chat.js at ddjp_392. A recorder, not a stub that swallows: if the panel stops
      // calling it, the assertion below has something to notice.
      ChatPanels: { _renderChatTierStrip: () => {}, _settingDisplayName: (k) => String(k) },
...
// in a settings row. The shape is cheap to check and the consequence is the entire screen.
{
  const src = require("fs").readFileSync(
    path.join(__dirname, "..", "ui", "settings.js"), "utf8");   // moved at ddjp_393

  // THE FIRST ARGUMENT IS A LITERAL KEY OR A NAME. The settings rework routes the on/off rows through
  // one `onOff(key, …)` and the per-group rows through `_renderFlagMapSetting`, both of which call
  // `optionRow(key, …)` with the key in a variable — a literal-only match lost those call sites and
  // this measurement its subject. So the name form is matched too, and "at least five" became
  // "EVERY call site classified": a floor that a smaller panel cannot pass by accident.
  const re = /optionRow\((?:optionRow,\s*)?(?:"([a-zA-Z]+)"|([A-Za-z_]\w*))[\s\S]{0,240}?\n\s*(\[\[|\[\{)/g;
  const pairs = [], objects = [];
  let m;
  while ((m = re.exec(src))) { (m[3] === "[[" ? pairs : objects).push(m[1] || m[2]); }
  const code = src.split("\n").filter((l) => !/^\s*\/\//.test(l)).join("\n");
  const callSites = (code.match(/\boptionRow\(/g) || []).length;

  ok(callSites >= 3 && pairs.length + objects.length === callSites,
    "APPLIED — EVERY `optionRow` call site was classified, or this measurement has no subject",
    { callSites, pairs, objects });
  ok(objects.length === 0,
...
// ═══ PART D — THE SKIP-ROADS CONTROL NEVER SENDS A LIST THE RULES WOULD DROP (ddjp_419) ═══════
// The reducer drops a malformed skip-road list WHOLESALE and says nothing, so the panel is the only
// place a person can be told. Found by J62: the panel offered a ninth road, clamped only the bottom
// of each count, and would send an empty list. This part EXTRACTS the real control, runs it against
// the REAL table from the REAL reducer, clicks it, and passes everything it sends back through the
// REAL `applySettingsEvent` — so "what the panel sends" and "what the rules accept" are one question.
{
  const { loadInContext } = require(path.join(__dirname, "_load.js"));
  const sd = loadInContext(["core/logger.js", "backends/backend1/ranks.js", "backends/backend1/consensushash.js",
    ...
    { Date, Math, JSON, setTimeout, clearTimeout, setInterval, clearInterval,
      localStorage: { getItem: () => null, setItem() {}, removeItem() {} }, window: {}, document: { body: { appendChild() {} } } }).StateDeriver;
  const LIM = sd.SETTING_RANGES.skipRoads;
  ok(!!LIM && typeof LIM.maxItems === "number", "D: PREMISE — the reducer declares the skip-road bounds", LIM);
  const fn = extract("_renderSkipRoadsSetting");
  ok(!!fn, "D: APPLIED — the skip-roads control must be extractable", "not found");

  // A DOM just rich enough for this control: it clears with firstChild/removeChild, reads `value`,
  // fires `onchange`, and shows an error through `style.display`.
  const mk = (tag, attrs, kids) => {
    const n = { tag, kids: [], cls: null, text: null, value: "", disabled: false, style: { display: "" },
    ...
    if (attrs) {
      ...
      if (attrs.text !== undefined) n.text = String(attrs.text);
      if (attrs.value !== undefined) n.value = attrs.value;
      if (attrs.style && /display:none/.test(attrs.style)) n.style.display = "none";
    }
    for (const c of (kids || [])) if (c) n.appendChild(c);
    return n;
  };
  const all = (n, out) => { out = out || []; for (const c of n.kids || []) { out.push(c); all(c, out); } return out; };
  function open(roads) {
    const painted = [], sent = [];
    const ctx = { Room: { getSettingRanges: () => ({ skipRoads: Object.assign({}, LIM) }) },
      H: { el: mk }, refs: { settingsBox: { appendChild: (n) => painted.push(n) } },
      _commitSetting: (k, blob) => sent.push(JSON.parse(JSON.stringify(blob))), _lockSetting: () => {} };
    vm.createContext(ctx);
    // The rework's row pieces, the REAL declarations out of ui/settings.js; `_box` is where to append.
    const layout = (extract("_rowHead") || "") + "\n" + (extract("_rowMore") || "") +
      "\nconst _box = () => refs.settingsBox; const _moreOpen = {}; function renderSettings() {}\n";
    vm.runInContext(layout + fn + ";globalThis.__f = _renderSkipRoadsSetting;", ctx);
    ctx.__f("skipRoads", "Skip rules", roads, true, "");
    const nodes = () => all(painted[0]);
    const button = (t) => nodes().find((x) => x.tag === "button" && x.text === t);
    const inputs = () => nodes().filter((x) => x.tag === "input");
    const err = () => nodes().find((x) => x.style && x.cls === "set-hint" && x.style.display === "block");
    return { sent, button, inputs, err, nodes };
  }
  const accepted = (blob) => JSON.stringify(sd.applySettingsEvent(sd.defaultSettings(), blob).skipRoads) === JSON.stringify(blob.skipRoads);

  // D1 — the cap. Click "Add a rule" far more often than the rules allow.
  const a = open([{ guestPlus: 1, vipPlus: 0 }]);
  for (let i = 0; i < LIM.maxItems + 5; i++) { const b = a.button("Add a rule"); if (b && !b.disabled) b.onclick(); }
  ok(a.inputs().length === LIM.maxItems * 2, "D1: the panel stops at maxItems (" + LIM.maxItems + ") roads, however often Add is pressed", a.inputs().length / 2);
  ok(a.button("Add a rule").disabled === true, "D1: and Add is DISABLED at the cap");
  a.button("Add a rule").onclick();
  ok(a.inputs().length === LIM.maxItems * 2, "D1: and pressing it anyway adds nothing");
  ok(a.nodes().some((x) => x.text === "Up to " + LIM.maxItems + " rules."), "D1: and the panel SAYS why");

  // D2 — the clamp, both ends, and the box shows what was kept.
  const inp = a.inputs();
  inp[0].value = String(LIM.itemMax + 50); inp[0].onchange();
  ok(inp[0].value === String(LIM.itemMax), "D2: a count above itemMax is clamped DOWN to it, visibly", inp[0].value);
  inp[1].value = String(LIM.itemMin - 7); inp[1].onchange();
  ok(inp[1].value === String(LIM.itemMin), "D2: a count below itemMin is clamped UP to it, visibly", inp[1].value);
  for (let i = 2; i < inp.length; i++) { inp[i].value = "3"; inp[i].onchange(); }
  a.button("Save rules").onclick();
  ok(a.sent.length === 1, "D2: a full, legal list is SENT", a.sent.length);
  ok(a.sent.length === 1 && accepted(a.sent[0]), "D2: and what the panel sent is what the REAL rules accept", a.sent[0]);

  // D3 — nothing to save: every road deleted. The rules need minItems, so this must not be sent.
  const b = open([{ guestPlus: 2, vipPlus: 0 }]);
  b.button("Remove").onclick();
  b.button("Save rules").onclick();
  ok(b.sent.length === 0, "D3: with no roads left, nothing is sent — the rules would drop it whole", b.sent);
  ok(!!b.err() && /at least/.test(b.err().text), "D3: and the panel SAYS so", b.err() && b.err().text);

  // D4 — only blank roads: they are filtered as "not a rule", which leaves the same empty list.
  const c = open([]);
  c.button("Add a rule").onclick(); c.button("Add a rule").onclick();
  c.button("Save rules").onclick();
  ok(c.sent.length === 0 && !!c.err(), "D4: blank roads only — nothing sent, and a message shown", { sent: c.sent });

  // CONTROL — an ordinary edit still saves, so the three refusals above are not a panel that refuses everything.
  const d = open([{ guestPlus: 5, vipPlus: 0 }]);
  d.button("Save rules").onclick();
  ok(d.sent.length === 1 && accepted(d.sent[0]) && !d.err(), "D: CONTROL — an ordinary list saves and is accepted", d.sent);
}
