...
// SUBJECT: backends/backend1/matrixbridge.js, backends/backend1/ranks.js
// `ranks.js` ADDED at `ddjp_384` — under-declared, and it is a textbook compares-two. The final part
// says so in its own words: each channel row's `level` is "written by hand and duplicate
// Ranks.LADDER", they "happen to agree today ... but nothing made them". TWO INDEPENDENTLY COMPUTED
// SIDES — `matrixbridge.js` holds literals and does not call `Ranks.levelOf` to derive them — so
// moving a rung in the ladder turns this red with nothing edited in the taxonomy. Same shape as
// `check-bot-runtime`'s `ranks.js`, and the opposite of `check-actions`, where one side is computed
// by calling the other and a change moves both together.
