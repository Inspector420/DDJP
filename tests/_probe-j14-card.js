...
// THE USER CARD moved to ui/roster.js at ddjp_391 — out of the FRONT of a banner span whose
// middle (the DM panel) stayed behind for ui/chat.js. This constant names the file the card is
// IN, which is not the file its banner is in. `check-dm-panel` reads `_CARD_ACTIONS` through
// this probe too, so both guards move with this one line.
const UI_REL = "ui/roster.js";
...
function driveCard(opts) {
  ...
  // The interim host seam (ui/base.js §host). ui/roster.js reaches its primitives through `H`
  // since ddjp_391, so the extracted opener needs it. Built FROM the stubs above rather than
  // beside them — two sets for one card are free to disagree about what `shortName` does.
  sandbox.H = {};
  for (const k of ["el", "clear", "shortName", "rankColor", "rankName", "_rosterLevel",
                   "avatarEl", "startCountdown", "clearCountdown", "_fitHeader", "_relock",
                   "_renderLeaveLock", "_openDMFromAction"]) {
    if (sandbox[k] !== undefined) sandbox.H[k] = sandbox[k];
  }
  sandbox.H.RANKS = () => sandbox.RANKS;
  sandbox.H.BOT_LEVEL = () => sandbox._BOT_LEVEL;
  sandbox.H.UPGRADE_DONE_PAUSE_MS = () => 600;
  sandbox.H.headerFit = () => (sandbox._headerFit || {});
