...
// SUBJECT: features/medialength.js, backends/backend1/statederiver.js
// BOTH, because REDUCER-INERT is a claim about the REDUCER and the WALL below says so in its second
// line. Declared single-path before compares-two (`ddjp_366`). PART E runs in its own sandbox loading
// `statederiver.js` and NOT `medialength.js`, and asserts across nowPlaying, rotation, settings,
// history and counts that folding `ddjp.media.len` moves none of them — including a floor-violating
// OWNER value, which is the case that would tempt a reducer into caring. For that to go false
// `statederiver.js` would have to start folding length reports into state. `capabilities.js` is
// loaded for the feature sandbox only and appears nowhere else in the file.
