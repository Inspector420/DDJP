...
// SUBJECT: backends/backend1/matrixbridge.js, features/userqueue.js, backends/backend1/streammanager.js, features/queue.js
// `features/queue.js` ADDED at `ddjp_384` — under-declared. PART D loads the REAL `Queue` and judges
// its gate: submitting while NOT live must send NOTHING ("the path that re-submitted during replay in
// a live room and split it in two"), and once live it must send normally. `MatrixBridge` is STUBBED
// there purely to capture the sends, so Queue is the accused and the bridge is the sink — the bridge
// earns its own place on this line from the SOURCE assertions at PARTs C and E instead.
// Why the other three are right and easy to misread: `matrixbridge.js` and `features/userqueue.js`
// are never loaded, they are read as SOURCE and asserted on; `streammanager.js` is aliased to `SM`
// at the top, so it appears zero times as `StreamManager.` anywhere near an `ok(`.
//
