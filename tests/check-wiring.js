...
// SUBJECT: index.html, features/queue.js, features/playback.js, backends/backend1/streammanager.js, backends/backend1/matrixbridge.js, backends/backend1/checkpoint.js
...
// in the file. Running it is the only honest test that a mechanism is reachable.
{
  ...
  // reasonable answer. It has to BE an answer, per site, rather than a file nobody looked at.
  {
    ...
    // `_awaitWritable(` joined at v389: the queue's gate stopped RETURNING a refusal and started
    // WAITING for liveness, because the message it logged ("will retry when live") named a mechanism
    // that did not exist and the act was dropped. The property this list checks — that a write is
    // gated before it happens — is unchanged; only the call's name is.
    const GATES = ["_mayWrite(", "_awaitWritable(", "MatrixBridge.mayAuthor(", "MatrixBridge.mayAdvance("];
  ...
  // other refused looks like from outside.
  {
    ...
    for (const m of q.matchAll(/async function (\w+)\(([\s\S]*?)\n  \}/g)) {
      ...
      const iAsk = Math.max(body.indexOf("_mayWrite"), body.indexOf("_awaitWritable"));
