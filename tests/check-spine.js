...
// ── AND ONE MODULE IN NO LAYER, BECAUSE IT SOLVES NO LAYER'S PROBLEM (J59) ──────────────────
// `PILLARS.md` §7 row 4: login and SSO, account switching, DM scope, avatars and recovery keys
// serve NEITHER pillar. J59 split them out of `matrixbridge.js` into `matrixaccount.js`. A module
// that serves neither pillar has no place in a story about the stream and the reading, and giving
// it one would make the story false in the other direction.
//
// THIS IS NOT A PATH EXEMPTION, AND THE DIFFERENCE IS THE WHOLE POINT. An exemption keyed by a
// file path is a claim about where something lives and goes stale the next time anything moves
// (52nd signature). Membership here is asserted by the PROPERTY that makes it true: a module
// serving neither pillar must not reach into the consensus stack AT ALL. Move one line of fold,
// order or protection work into it and PART A2 turns red — which is `PILLARS.md` §7's boundary
// rule made drivable rather than restated.
const NEITHER = ["matrixaccount"];
const CONSENSUS_STACK = ["StreamManager", "EventCache", "StateDeriver", "Capabilities", "Dials",
  "History", "Continuity", "Session", "Vouch", "TrustPolicy", "Checkpoint", "CheckpointEngine",
  "Floor", "CheckpointFormat", "SettingsProof", "ConsensusHash", "Scheduler"];
...
// ── PART A — the layers are real, and they account for the whole backend ────────────────────
{
  ...
  const orphans = onDisk.filter((m) => placed.indexOf(m) < 0 && NEITHER.indexOf(m) < 0);
  ok(orphans.length === 0,
...
// ── PART A2 — the modules in NO layer really serve neither pillar ───────────────────────────
// Driven by the property, not by the name. A `NEITHER` module that starts touching the consensus
// stack has stopped being plumbing, and the layer story has to gain it or the module has to give
// the work back.
{
  const onDisk = fs.readdirSync(BE).filter((f) => f.endsWith(".js")).map((f) => f.replace(/\.js$/, ""));
  for (const m of NEITHER) {
    ok(onDisk.indexOf(m) >= 0,
      "A2: APPLIED — `" + m + "` is named as serving neither pillar and exists in the tree", onDisk);
    const placedNames = [];
    for (const L of LAYERS) for (const x of L.mods) placedNames.push(x);
    ok(placedNames.indexOf(m) < 0,
      "A2: APPLIED — `" + m + "` is in NO layer. Naming it in one would say it solves a problem the "
      + "six layers are an ordering of, and PILLARS.md §7 says it serves neither pillar");
    const src = fs.readFileSync(path.join(BE, m + ".js"), "utf8")
      .replace(/(^|[^:"'`\\])\/\/.*$/gm, "$1");          // comments only; a name in prose is not a reach
    const reaches = CONSENSUS_STACK.filter((g) => new RegExp("(?<![\\w$.])" + g + "(?![\\w$])").test(src));
    ok(reaches.length === 0,
      "A2: APPLIED — `" + m + "` reaches NOTHING in the consensus stack, which is what makes it "
      + "plumbing rather than a layer. If this is red, either the work belongs in a layer or the "
      + "module has taken work that is not its own (PILLARS.md §7)", reaches);
  }
}
...
// green [spine] as covering it.
{
  ...
  // `backends/backend2/runner.js` joined at J24. It is the only timer that does NOT tick in every
  // client: it runs in the single client holding the ladder's top rung exactly — the runner of a
  // bot-run room — and is loaded and idle everywhere else. Declared in the same place as the rest,
  // because "only sometimes" is still behaviour a page has to describe.
  const TIMER_OWNERS = ["backends/backend1/matrixbridge.js", "backends/backend1/scheduler.js",
                        "backends/backend1/session.js", "backends/backend2/runner.js"];
...
console.log("[spine] PASS — the layer story is true of the tree: every backend module is placed in "
  ...
  + "someone to say which problem it solves — or is named as serving NEITHER pillar, which is "
  + "asserted by the property rather than by a path: `matrixaccount` reaches nothing in the "
  + "consensus stack, and one line of fold, order or protection work moved into it turns PART A2 "
  + "red (PILLARS.md §7); the dependencies the story rests on are real module "
