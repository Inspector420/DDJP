...
// SUBJECT: core/, features/, ui/, app.js
...
// `transport/` WAS IN THIS LIST AND DOES NOT EXIST. `listJs()` returns [] for a missing
// directory, so it contributed nothing and said nothing — a scan target that reads as coverage
// while adding none. The contents moved into `backends/backend1/`, which the real entries
// already reach. Removed rather than repointed: this guard's population is the DOM/app layers,
// and the file count it prints is unchanged by the removal, which is the proof it was dead.
//
// AND `backends/backend1` WAS MISSING, WHICH MADE THREE ENTRIES OF `SECRET_IDS` DEAD FROM THE DAY
// THEY WERE WRITTEN. `_ssKey`, `_loginPassword` and `_pendingNewKey` have only ever been declared
// in `backends/backend1/matrixbridge.js` — a directory this population did not contain — so those
// three names could never match, in a list that reads as covering them. That is the catalogue's
// OLDEST entry (a search that has never matched anything has not been tested) sitting directly
// below a comment about removing a different dead scan target. Found at J59, which moved those
// three declarations into `matrixaccount.js`: a file whose entire job is handling secrets and
// which makes 34 `Logger.*` calls. The population is the app AND the backend now.
const FILES = [
  ...listJs("core"), ...listJs("features"), ...listJs("ui"), ...listJs("backends/backend1"),
