...
// SUBJECT: backends/backend1/statederiver.js, backends/backend1/matrixbridge.js, backends/backend1/consensushash.js, backends/backend1/checkpointformat.js, docs/HANDOFF.md
//
...
assert.ok(
  ...
  "      carries the break and its acknowledgement together — AND open the state index with a 🔴 line\n" +
  "      naming what breaks and what a room must do, carrying the new room-surface id (front door, step 9).");

// ═══ AND THE PERSON RECEIVING THE PACKAGE IS TOLD (owner ruling, ddjp_417) ═══════════════════
// Detecting a break is not the same as the developer hearing about it: a BASELINE update is a line in
// a diff nobody reads. So the state index must say, in its opening lines and exactly once, whether this
// package breaks existing rooms — 🔴 naming what breaks and what a room must do, or 🟢 when nothing does
// — beside the room-surface id computed here. When the surface moves, the recorded id stops matching
// and this goes red until the state index is re-said. It cannot make a session choose 🔴 honestly; it
// makes the choice impossible to skip, which is what "never silent" needs.
// The id is an ORDER-STABLE hash of the derived surface. Not the `sig` below: that one passes a key
// list to JSON.stringify, which also FILTERS nested keys — harmless while every surface is a list of
// strings, and silently blind the day one holds objects.
function stable(v) {
  if (Array.isArray(v)) return "[" + v.map(stable).join(",") + "]";
  if (v && typeof v === "object") return "{" + Object.keys(v).sort().map((k) => JSON.stringify(k) + ":" + stable(v[k])).join(",") + "}";
  return JSON.stringify(v);
}
const SURFACE_ID = crypto.createHash("sha256").update(stable(now)).digest("hex").slice(0, 10);
function markerVerdict(text, id) {
  if (typeof text !== "string") return { ok: false, why: "the state index could not be read" };
  const head = text.split("\n").slice(0, 40);
  const marks = head.filter((l) => /^(🔴|🟢)/u.test(l));
  if (marks.length !== 1) return { ok: false, why: "its opening lines carry " + marks.length + " 🔴/🟢 line(s); exactly one is required" };
  const m = marks[0].match(/`([0-9a-f]{10})`/);
  if (!m) return { ok: false, why: "the 🔴/🟢 line carries no room-surface id in backticks" };
  if (m[1] !== id) return { ok: false, why: "it names room surface `" + m[1] + "` and the surface is now `" + id + "`" };
  return { ok: true, kind: marks[0].startsWith("🔴") ? "breaks" : "no-break" };
}
// THE READER IS UNTESTED CODE, SO IT IS SELF-TESTED FIRST — a checker that passes everything would
// certify every state index on its own authority.
{
  const good = "# t\n\n🟢 No change breaks rooms. Surface `" + SURFACE_ID + "`.\n";
  const cases = [
    [good, true], [good.replace("🟢", "🔴"), true],
    ["# t\n\n🟢 No change. Surface `0000000000`.\n", false],
    ["# t\n\nno marker at all\n", false],
    [good + "🔴 a second one `" + SURFACE_ID + "`\n", false],
    ["# t\n\n🟢 a marker with no id\n", false],
  ];
  for (const [text, want] of cases) {
    assert.ok(markerVerdict(text, SURFACE_ID).ok === want,
      "[room-compat] FAIL — the state-index marker reader is wrong about its own fixture: " + JSON.stringify(text));
  }
}
const { docPaths } = require(path.join(__dirname, "_docs.js"));
const fsx = require("fs");
let handoff = null;
for (const p of docPaths("HANDOFF.md")) if (fsx.existsSync(p)) { handoff = fsx.readFileSync(p, "utf8"); break; }
const mv = markerVerdict(handoff, SURFACE_ID);
assert.ok(mv.ok,
  "[room-compat] FAIL — THE STATE INDEX DOES NOT SAY WHETHER THIS PACKAGE BREAKS EXISTING ROOMS: " + mv.why +
  ".\n      Breaking old rooms is ALLOWED while pre-release; it is never silent. Open the state index with ONE\n" +
  "      line: 🔴 naming what breaks and what a room must do, or 🟢 when nothing does — carrying `" + SURFACE_ID + "`.");
...
console.log(
  ...
  " surfaces, the missing one named above (signature " + sig + "). And the state index SAYS whether this " +
  "package breaks existing rooms — " + (mv.kind === "breaks" ? "🔴 it does" : "🟢 it does not") + " — against " +
  "room surface `" + SURFACE_ID + "`, its reader self-tested first");
