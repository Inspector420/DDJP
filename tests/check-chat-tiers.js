...
// SUBJECT: features/chat.js, ui/chatbuffer.js, ui/chat.js
...
//
// RE-AIMED AT ddjp_425. This part extracted five state functions by name (`_newChatState`,
// `_chatStates`, `_chatState`, `_allChatStates`, `_resetChatState`) and drove them in isolation.
// Chat was rebuilt model-first then — one buffer per CHANNEL ID on a model owned by the box — and
// those five are gone, so the part now drives the REAL `ui/chat.js` through `_chat-world`, with the
// switch made the way a person makes it: by clicking the strip. The property is unchanged, and it is
// now asserted of the file rather than of pieces lifted out of it.
{
  const W = require("./_chat-world");
  const w = W.world();
  for (let i = 0; i < 40; i++) w.raw("$u" + i, "!u:hs", 1000 + i);
  for (let i = 0; i < 7; i++) w.raw("$s" + i, "!s:hs", 2000 + i);
  const bufs = w.box._chat && w.box._chat.bufs;
  const main = bufs && bufs["!u:hs"], staff = bufs && bufs["!s:hs"];
  ok(main && staff && main.size() === 40 && staff.size() === 7,
    "A: APPLIED — both tiers must hold messages before the switch, or retention below is free",
    { main: main && main.size(), staff: staff && staff.size() });

  const click = (label) => { const btn = W.tierButton(w, label); ok(!!btn, "A: APPLIED — a '" + label + "' button to click", null); btn.onclick(); };
  click("Staff");
  click("Everyone");
  ok(w.box._chat.bufs["!u:hs"] === main && main.size() === 40,
    ...
    { same: w.box._chat.bufs["!u:hs"] === main, size: main.size() });
  click("Staff");
  ok(w.box._chat.bufs["!s:hs"] === staff && staff.size() === 7,
    "A: in both directions, so retention is a property of the structure rather than of one tier",
    staff.size());

  // THE CONTROL, AND IT IS THE OTHER HALF OF THE RULE. A ROOM change must still start clean. The
  // model is owned by the box, and every room entry builds a NEW box (`buildMainDom`), so the next
  // room's chat has none of this one's buffers — retention above cannot be a clear that stopped
  // working, because the clear is visible here.
  w.refs.chatBox = W.el("div");
  w.raw("$next0", "!u:hs", 3000);
  const fresh = w.refs.chatBox._chat && w.refs.chatBox._chat.bufs["!u:hs"];
  ok(fresh && fresh !== main && fresh.size() === 1 && main.size() === 40,
    "A control: a ROOM change starts clean — the new room's box holds a new buffer with only the new " +
    "room's message, and the old room's buffer is untouched and unreachable from it. Clearing was never " +
    "wrong; before J12 it was the response to the wrong event, and here a room change clears while a " +
    "tier switch never does",
    { fresh: fresh && fresh.size(), old: main.size() });
}
