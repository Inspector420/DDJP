...
// SUBJECT: features/botsettings.js, features/botruntime.js, ui/settings.js, backends/backend1/matrixbridge.js
...
function finish() {
  ...
  // rest of this file exists to avoid.
  {
    ...
    ok(/BotSettings\.request/.test(code),
      ...
      + "unreachable");
    // ── RE-ANCHORED AT J31, AND THE PROPERTY IS STRICTLY STRONGER THAN THE OLD REGEX ─────────
    // This pinned the literal `return Room.setSettings(partial);`. J31 routed the owner write
    // through `Actions.perform("room.settings", …)` — the adapter, NOT the bot — and the literal
    // stopped matching while the property this part names was untouched: `perform` calls
    // `Room.setSettings(a.partial)` and never touches `BotSettings`.
    //
    // **A GUARD MODIFIED SO A CHANGE CAN PASS IS THE MOST DANGEROUS EDIT AVAILABLE**, so the
    // replacement asserts MORE than the regex did rather than less. The old one said the branch
    // looked a particular way; this says what the branch must and must not REACH — which is the
    // sentence below the assertion, and is what a future rewrite in a third shape still has to
    // satisfy. Driven: pointing the write branch at `BotSettings.request` turns this red.
    // EXTRACTED AS A BLOCK, NOT AS A LINE. The first version of this took `[^\n]*` — one line —
    // and broke the moment the branch grew a `.catch`, reporting "does not dispatch the settings
    // write at all" about a branch that plainly does. That is the SAME brittleness as the literal
    // regex this replaced, arriving one shape later: a guard that pins layout rather than
    // behaviour fails on a reformat and passes on a rewrite. Braces are balanced when the branch
    // is a block, so the assertion reads the whole branch however it is written.
    const writeBranch = (function () {
      const at = code.indexOf('if (how === "write")');
      if (at < 0) return "";
      const brace = code.indexOf("{", at);
      const nl = code.indexOf("\n", at);
      if (brace < 0 || (nl >= 0 && nl < brace)) return code.slice(at, nl < 0 ? code.length : nl);
      let depth = 0;
    ...
    ok(writeBranch.length > 0,
      "C: APPLIED — the `how === \"write\"` branch is gone from the panel entirely, so the rows "
      + "below are asserting nothing about the owner's path", code.slice(0, 200));
    ok(!/BotSettings/.test(writeBranch) && !/\brequest\b/.test(writeBranch),
      "C: the owner's write branch reaches `BotSettings` / a request. Delegation must not put the "
      + "owner's own settings through a bot that may not be running", writeBranch);
    ok(/Room\.setSettings\s*\(|Actions\.perform\(\s*"room\.settings"/.test(writeBranch),
      "C: the owner's write branch does not dispatch the settings write at all — by either the "
      + "direct call or the adapter route", writeBranch);
    ok(true,
    ...
    // Retitled in the settings rework ("Bot permissions" section); the RULE is unchanged.
    const delIdx = code.indexOf('_renderDelegationSetting("What others can ask the bot to change"');
    // THE 49TH SIGNATURE. `_settingsLocked` is owned by ui/interface.js and read here through the
    // host seam since ddjp_393, so the bare name became `H.settingsLocked()`. The RULE did not
    // change — the grammar did, because the value now lives in another file. Both matched.
    ok(delIdx > 0 && /isOwner && !(?:_settingsLocked|H\.settingsLocked\(\))/.test(code.slice(delIdx, delIdx + 260)),
