...
(async () => {
  ...
  const seenMsgs = [];
  Chat.onMessage((id, sender, body, failed, ts) => seenMsgs.push({ id, sender, body, ts }));
  Chat.init("room1");
  ok(typeof captured === "function", "init registers a raw listener");
  captured({ type: "m.room.message", room_id: "room1", event_id: "e_live", sender: "@a:hs", content: { body: "hi" }, ts: 1700000000000 }, {}, {});
  eq(seenMsgs, [{ id: "e_live", sender: "@a:hs", body: "hi", ts: 1700000000000 }], "live message forwarded with event_id AND ts (window orders by ts)");
  ...
  calls = 0;
  nextResult = { messages: [
    { event_id: "o1", sender: "@a", body: "first",  ts: 100 },
    { event_id: "o2", sender: "@b", body: "second", ts: 200 },
    { event_id: "o3", sender: "@c", body: "third",  ts: 300 }
  ] };
  let r = await Chat.backfillRecent(10);
  eq(r.messages.map(m => m.id), ["o1", "o2", "o3"], "backfill returned oldest->newest");
  eq(r.messages.map(m => m.ts), [100, 200, 300], "ts carried per message (buffer's chronological order key)");
