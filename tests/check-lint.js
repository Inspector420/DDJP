// tests/check-lint.js
// SUBJECT: backends/backend1/, core/, features/, ui/, tests/_load.js
...
ok(Object.keys(EXEMPTIONS).length === 1,
  "EXACTLY ONE EXEMPTION EXISTS — the relaxed empty-catch rule — and the list is asserted " +
  "exhaustive so a new one cannot join silently. (There were two until ddjp_425: the PARKED " +
  "`_resetChatState` finding left with the binding it waived.)", Object.keys(EXEMPTIONS));
...
// from KNOWN_GLOBALS, so the list is loaded either way and a second reader would be a second copy.
{
  ...
  // DIGITS ALLOWED. The seam's engine-scoped names (`B1StreamManager`) contain one, and
  // `[A-Za-z]+` would have read that entry as two names, "B" and "StreamManager" (J23).
  const known = new Set([...m[1].matchAll(/"([A-Za-z][A-Za-z0-9]*)"/g)].map((x) => x[1]));
  ...
  // BACKEND DIRS ARE DERIVED, NOT LISTED. This said "backends/backend1" until J23. The moment a
  // second engine had a `.js` in it, a hard-coded entry here would have been a wall that covered
  // one module less than it claimed — the forty-ninth signature, in the file that exists to catch
  // exactly that. Same derivation `check-boundaries` uses: subdirectories of `backends/` holding
  // at least one `.js`.
  const backendDirs = fs.readdirSync(path.join(ROOT, "backends"), { withFileTypes: true })
    .filter((e) => e.isDirectory() &&
      fs.readdirSync(path.join(ROOT, "backends", e.name)).some((f) => f.endsWith(".js")))
    .map((e) => "backends/" + e.name);
  const mods = ["core", ...backendDirs, "features", "ui"].reduce((a, d) => walk(d, a), []);
  ...
  for (const rel of mods) {
    ...
    const d = src.match(/^const ([A-Z][A-Za-z0-9]*) = \(\(\) =>/m);
