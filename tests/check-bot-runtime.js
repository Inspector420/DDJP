...
// SUBJECT: features/botruntime.js, backends/backend1/ranks.js
...
function partE() {
  ...
  ok(modes.length === 1 && modes[0] === "consensus",
    "E: EXACTLY ONE MODE EXISTS — and the REASON changed at v392 while the count did not. It was " +
    "`backend-selection.md` forbidding a seam before a second engine exists. That engine now " +
    "EXISTS (`backends/backend2/`, J24), so the row stands on a measured reason instead: " +
    "`bot-backend.md` §5d/§8 swept this module against bot mode and marked it [checked] — it is " +
    "the AFK and presence half, shared and carried over unchanged, while the authority half is a " +
    "separate program in the engine. A second row is UNNECESSARY, no longer unbuildable", modes);
  ...
  // ── REGRESSION, v392 ──────────────────────────────────────────────────────────────────────
  // This row's reason USED TO READ "the only room type that exists". That was true until the
  // bot-run engine shipped and false from v392 on, and nothing caught it: the string is exported
  // data, not a comment, and every assertion here was satisfied by any string over 20 chars.
  // The claim is pinned NEGATIVELY because the honest reason will keep being reworded and only
  // the exclusivity claim is actually forbidden.
  ok(!/\bonly room type\b|\bthe only mode\b|\bno other room type\b/i.test(RT.MODES.consensus.why),
    "E: and that reason does NOT claim to be the only room type. It exists — `backends/backend2/`, " +
    "J24, v392 — and this row covers BOTH, because `bot-backend.md` §5d/§8 swept this module " +
    "against bot mode and marked it [checked] as shared and carried over unchanged",
    RT.MODES.consensus.why);

  // The row's NAME is narrower than what it covers, and is kept rather than renamed — so the
  // explanation is the thing that has to survive. `roles.md` §6 owns the four-way split of the
  // word; this asserts the file still points at it, because a comment nobody is required to keep
  // is a comment that goes.
  const rawSrc = fs.readFileSync(path.join(ROOT, "features/botruntime.js"), "utf8");
  ok(/roles\.md/.test(rawSrc) && /Confusable/i.test(rawSrc),
    "E: and the file carries the NAME warning, citing `roles.md` §6 *Confusable pairs* — the word " +
    "`consensus` means four things here (the P1 principle, the docs directory, ANY registered " +
    "engine in `core/backends.js`, and this row), and this is the one that reads as a room type " +
    "and is not one: the seam registers `backend1` / `backend2`, never `consensus`",
    "names roles.md and Confusable");
