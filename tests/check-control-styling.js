...
// ── READ AS THE ui/ POPULATION (widened at ddjp_390, BEFORE the clusters that build controls) ─
// THE THIRD AND LAST OF THE PRECONDITIONS RECORDED AT ddjp_386, and the only one widened AHEAD of
// the break rather than on it. This guard asserts every interactive element the UI builds is
// styled by something the stylesheet declares — a whole-file NEGATIVE property, which is the class
// that stops covering code that leaves the file WITHOUT GOING RED. The player cluster built zero
// controls so it lost nothing; `ui/roster.js` builds the rank select and the card actions, so the
// widening happens now.
//
// THE 49TH SIGNATURE NEARLY BIT HERE AND THE ONLY REASON IT DID NOT IS AN ACCIDENT OF STYLE.
// After the split a cluster reaching the primitive through the host writes `H.el("button"` rather
// than `el("button"`. The detector below searches with `indexOf('el("button"')` — a plain
// SUBSTRING scan — so the member form matches without anything being added. Had it used a
// word-boundary regex, widening the population would have silently stopped seeing every control
// in every extracted file: the guard reading as strictly improved while covering less.
// Driven at ddjp_390 rather than reasoned about: an unstyled control planted in ui/playlists.js
// turns this red. The luck is recorded because the next guard widened this way may not have it.
...
ok(UI_FILES.length >= 3,
  "APPLIED — the ui/ population must hold the split files, or every control this guard judges is " +
  "a control it cannot see. Found: " + UI_FILES.join(", "), UI_FILES);
