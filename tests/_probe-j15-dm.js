...
// The chat surfaces moved to ui/chat.js at ddjp_392 — including THE DM PANEL, which came out of
// the middle of what used to be the roster's banner span. This constant names the file they are
// IN rather than the file their banner used to be in.
...
function driveRoute(opts) {
  ...
  // The interim host seam (ui/base.js §host). ui/chat.js reaches its primitives through `H` since
  // ddjp_392. Built FROM the stubs above rather than beside them — two sets for one panel are
  // free to disagree about what `shortName` does.
  sandbox.H = {};
  for (const k of ["el", "clear", "shortName", "rankColor", "_rosterLevel", "avatarEl",
                   "_relockAllPanels"]) {
    if (sandbox[k] !== undefined) sandbox.H[k] = sandbox[k];
  }
  sandbox.H.rightTab = () => "dm";
  sandbox.H.queueTab = () => "room";
...
// `_fmtAgo` lives in ui/panels.js since ddjp_387 and the DM row calls it. Extracted by the same
// machinery as the panel itself, so a rename in that file refuses here rather than passing with a
// stub standing in for the real formatter.
function _fmtAgoSrc() {
  const ex = extractNamed("ui/panels.js", "_fmtAgo");
  if (!ex.ok) throw new Error("_fmtAgo not extractable from ui/panels.js — " + ex.stage);
  return ex.source;
}

function drivePanel(opts) {
  const o = opts || {};
  // `_dmMatches`, `_renderDMList`, `_dmRowEl` joined at ddjp_433 — the list is a search box plus one row per person.
  const pieces = ["_dmFoldMessage", "_renderDMBadge", "renderDMPanel", "_openDMConversation", "_dmMatches", "_renderDMList", "_dmRowEl", "_dmStartable", "_dmStartRow", "_dmSuggestions", "_dmSuggestRow"];
  const srcs = [];
  ...
  // Same host seam as the first driver — one definition, two drivers.
  sandbox.H = {};
  for (const k of ["el", "clear", "shortName", "rankColor", "_rosterLevel", "avatarEl",
                   "_relockAllPanels"]) {
    if (sandbox[k] !== undefined) sandbox.H[k] = sandbox[k];
  ...
  try {
    vm.runInContext(
      ...
      // `Panels` — the DM row shows a relative timestamp through `Panels._fmtAgo`, which moved
      // to ui/panels.js with ROOM HISTORY at ddjp_387. The REAL function is extracted from that
      // file and injected rather than stubbed: a stub would let the panel render while the thing
      // it actually calls was broken, which is the shape this probe exists to refuse.
      "const Panels = { _fmtAgo: (" + _fmtAgoSrc().replace(/^\s*function\s+_fmtAgo/, "function") + ") };\n" +
      srcs.join("\n") +
  ...
  return {
    ...
    badge: tabDM.text,
    badgeUnread: tabDM.classList.contains("mentioned"),   // the purple dot (ddjp_432)
    fold: sandbox.__fold,
