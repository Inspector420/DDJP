...
// SUBJECT: features/userqueue.js, features/playlists.js, backends/backend1/statederiver.js
// `features/queue.js` REMOVED at `ddjp_384` — over-declared. It is NEVER LOADED: the five sandboxes
// in this file are statederiver (x3), userqueue and playlists. Its only appearance outside the old
// SUBJECT line is the note in `finish()` saying `Queue.recentHistory` is STUBBED "so mutating any of
// them in their own module leaves this file green" — the author stating a non-subject outright.
// Positive control before trusting that: `grep -ic queue` matches 33 times here, so the query is
// live and the absence is the module's, not the matcher's.
...
function botTree(opts) {
  ...
  let raw = null, ready = null, caughtUp = false;
  const sb = loadInContext([
    "core/logger.js", "backends/backend1/ranks.js", "backends/backend1/consensushash.js",
    "backends/backend1/trustpolicy.js", "backends/backend1/statederiver.js",
    ...
    MatrixBridge: {
      ...
      getRoster: () => [],
      // CAPTURED, so the handler the runtime really registers can be driven. Production reaches
      // `sweepRepeat` through this and through the tick, and neither has a name to extract — so
      // without this the event-driven half could only be asserted as source text.
      onRawEvent(fn) { raw = fn; }, offRawEvent() {},
      joinedMembersOf: () => [], invitedMembersOf: () => [],
      // SHAPED LIKE THE REAL ONE: an OBJECT with `.ok`, never a boolean. The first harness
      // returned a bare boolean, so a subject reading it with `!!` passed while gating nothing —
      // the fixture agreeing with the bug instead of exposing it.
      mayAuthor: () => ({ ok: (o.behindAtStart ? caughtUp : (("mayAuthor" in o) ? o.mayAuthor : true)) }),
      // CAPTURED like the raw handler, and for the same reason: production reaches the sweep
      // through this edge and it has no name to extract.
      onAuthorReady(fn) { ready = fn; },
    },
  ...
  return { sb, calls, started, np2, raw: (e) => raw && raw(e, null, null),
           live: () => { caughtUp = true; }, ready: () => ready && ready() };
}
function partH() {
  ...
  return new Promise((res) => setImmediate(res)).then(() => {
    ...
    // ── THE SKIP FOLLOWS THE EVENT, NOT THE MINUTE TICK ────────────────────────────────────
    // REPORTED FROM A ROOM: the repeat played on for a long time first. The sweep rode the tick
    // alone, so the delay was whatever was left of a minute — for a rule whose whole point is that
    // the song should not be playing. It now runs from the bot's raw-event hook as well, which is
    // safe because `_ingestSpineEvent` folds BEFORE it fans out to raw listeners, so `nowPlaying`
    // is already the new song. Driven through the REAL handler the transport was given rather than
    // by calling the sweep, because "the sweep is right" and "something invokes it" are the two
    // questions this tree keeps finding apart.
    const t5 = botTree({ np: NP, verdict: BLOCKED });
    ok(typeof t5.raw === "function",
      "H: APPLIED — the runtime registered a raw-event handler with the transport", typeof t5.raw);
    ok(t5.calls.skips.length === 0,
      "H: APPLIED — and nothing has been skipped before the event arrives", t5.calls.skips);
    t5.raw({ ddjpType: "ddjp.dj.play", event_id: "$adv", sender: "@a:hs" });
    ok(t5.calls.skips.length === 1 && t5.calls.skips[0] === "repeat",
      "H: one folded event is enough — the skip does not wait for the next tick", t5.calls.skips);

    // AND AN EVENT THAT IS NOT AN ADVANCE STILL DRIVES IT, which is why no advance type is named
    // at the hook: `ddjp.media.skip` has been left off six subscription lists across four
    // sessions, and a rule keyed on "something was folded" cannot become the seventh.
    const t6 = botTree({ np: NP, verdict: BLOCKED });
    t6.raw({ ddjpType: "ddjp.dj.vote", event_id: "$v", sender: "@a:hs" });
    ok(t6.calls.skips.length === 1,
      "H: and it is not keyed on the event TYPE, so no advance type can be forgotten here",
      t6.calls.skips);

    // ── AND THE THIRD PATH: THE MOMENT THIS CLIENT MAY ACT AT ALL ──────────────────────────
    // The gap the other two leave. The bot is started during WIRING, before replay, so
    // `mayAuthor()` is false for every replayed event — including the advance that put the song on
    // air. A quiet room then folds nothing and the first chance to act is the next tick. That is
    // exactly "joined a room where a repeat is already playing". `onAuthorReady` fires on the
    // transition to LIVE, which is the interface's own released-from-parking hook and the one
    // `UserQueue` already uses.
    const t7 = botTree({ np: NP, verdict: BLOCKED, behindAtStart: true });
    ok(typeof t7.ready === "function",
      "H: APPLIED — the runtime registered a released-on-LIVE handler", typeof t7.ready);
    ok(t7.calls.skips.length === 0,
      "H: APPLIED — and while it is behind, nothing is skipped whatever arrives", t7.calls.skips);
    t7.live();
    t7.ready();
    ok(t7.calls.skips.length === 1,
      "H: becoming caught up drives the sweep, so a bot that joined into a repeat does not wait "
      + "for the tick in a room that has gone quiet", t7.calls.skips);
