...
function session(initial, mbCalls) {
  ...
  return loadInContext(["core/storageio.js", "core/store.js", "backends/backend1/ranks.js", "backends/backend1/capabilities.js", "features/room.js"], { localStorage, Logger, MatrixBridge });
}

const PARTIAL = {
  name: "Party",
  spaceId: "!space:hs",
  channels: { events_uncategorized: "!a:hs", events_owner: "!b:hs", checkpoints_uncategorized: "!c:hs" },
};
...
//    BRAND-NEW session (memory empty) purely from persisted storage.
{
  ...
  eq(p.total, 6, "surfaced total (batch-1 channel count from the taxonomy)");
}
