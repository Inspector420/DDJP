...
{
  // THE PREMISE MOVED WITH `CONSTANTS`, IN THE SAME CHANGE, AS RECORDED AT ddjp_386.
  // `RANKS` is declared in ui/base.js since the final cut. This was flagged as a precondition on
  // whichever cluster took PRIMITIVES and CONSTANTS eleven packages before it came due — and it is
  // the reason ui/base.js was held at 72 lines rather than taking them early: moving them then
  // would have pulled a tenth guard into a nine-guard change.
  const src = fs.readFileSync(path.join(ROOT, "ui/base.js"), "utf8");
  const at = src.indexOf("const RANKS =");
  ok(at > 0, "PREMISE: ui/base.js still declares RANKS — if this fails the rows below pin nothing");
