...
// SUBJECT: features/room.js, features/actions.js, ui/base.js
...
{
  // PART E SPANS TWO FILES SINCE ddjp_391 AND THAT IS THE HONEST SHAPE, not a swap.
  // `renderMyRank` moved to ui/roster.js with RANK AND AVATAR; `_BOT_LEVEL`'s derivation from the
  // ladder and `rankName` stayed in ui/interface.js with CONSTANTS and PRIMITIVES. Re-pointing at
  // roster alone made the two halves that stayed behind look absent — a guard failing for a reason
  // unrelated to its subject. Both are read, and the assertions below are about the PROPERTY
  // rather than about which file happens to hold each half today.
  const ui = ["ui/base.js", "ui/shell.js", "ui/roster.js"]
  ...
  // THE 49TH SIGNATURE, LIVE. `rankSelect` moved to ui/roster.js at ddjp_391 while `_BOT_LEVEL`
  // stayed in ui/interface.js with CONSTANTS — so inside the moved function the bare name became
  // `H.BOT_LEVEL()`, and a matcher written for the bare form stopped seeing a rule that had not
  // changed at all. The grammar moved, not the property. Both forms are matched, deliberately.
  ok(/(?:_BOT_LEVEL|H\.BOT_LEVEL\(\))/.test(sbody) && /appoint/.test(sbody),
