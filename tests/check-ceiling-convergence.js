...
// SUBJECT: features/playback.js, backends/backend1/statederiver.js
// BOTH, because the guard exists BECAUSE the two halves must agree (compares-two, `ddjp_366`; this
// row predates it). `playback.js` AUTHORS the ceiling advance and the REDUCER resolves N competing
// ones to exactly ONE — the PASS line's own last clause. The second half runs in its own sandbox
// loading `statederiver.js` and not `playback.js`, and asserts on `SD.derive`: the rotation moves off
// the genesis pi and history holds exactly TWO plays rather than N+1. For that to become false the
// reducer's advance-lock would have to change, so the advance-lock is on trial here, not a vehicle.
