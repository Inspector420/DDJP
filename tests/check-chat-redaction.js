...
// SUBJECT: backends/backend1/matrixbridge.js, backends/backend1/statederiver.js, features/chat.js
...
//   PART D — the removal routes to the CHANNEL the message is IN, and a message never held is a
...
// ═══ PART D — the removal routes by CHANNEL, tombstones in place, and a never-held message is a no-op
// RE-AIMED AT ddjp_425. This part drove `removeChatMessage` extracted from the file, seeding per-TIER
// buckets through `_chatState(box, tier)` and counting "mounted ids". Chat was rebuilt model-first —
// one buffer per CHANNEL ID, and the box a rendering of `ChatBuffer.view` — so those pieces are gone
// and the part drives the REAL `ui/chat.js` through `_chat-world`. Every property below is the one
// this part asserted before, now read off the screen and the model rather than off a stubbed DOM.
// The `expectSeeded` admissibility the harness used to enforce is an APPLIED premise in each block.
{
  const W = require("./_chat-world");
  const seen = (w) => W.screen(w.box);
  const rec = (w, ch, id) => { const b = w.box._chat && w.box._chat.bufs[ch]; return b ? b.get(id) : null; };
  const click = (w, label) => { const b = W.tierButton(w, label); ok(!!b, "D: APPLIED — a '" + label + "' button", null); b.onclick(); };

  // The message is in the VISIBLE tier and on screen.
  {
    const w = W.world();
    w.raw("$a", "!u:hs", 100, "body-of-$a"); w.raw("$b", "!u:hs", 101, "second");
    click(w, "Everyone");
    ok(seen(w).join() === "m:$a,m:$b", "D visible: APPLIED — both rows are on screen before the redaction", seen(w));
    w.redaction("$a", "!u:hs");
    ok(w.box._chat.bufs["!u:hs"].ids().join() === "$a,$b",
      "D: THE ROW STAYS AND KEEPS ITS SLOT. A vanished row is indistinguishable from one that was " +
      "never there, so a deletion leaves a mark — and it leaves it where the message was", w.box._chat.bufs["!u:hs"].ids());
    ok(rec(w, "!u:hs", "$a").redacted === true && rec(w, "!u:hs", "$a").body === "",
      "D: marked as redacted, with no body left behind — the tombstone is a removal of the text, " +
      "not a decision about how to draw it", rec(w, "!u:hs", "$a"));
    ok(rec(w, "!u:hs", "$b").redacted === false,
      "D control: and its neighbour is untouched, so the transition applies to one record rather " +
      "than to the tier", rec(w, "!u:hs", "$b"));
    const row = w.box.children[0];
    ok(seen(w).join() === "m:$a,m:$b" && /Message deleted/.test(row.textContent),
      "D: AND THE ROW ON SCREEN BECOMES THE TOMBSTONE IN PLACE rather than leaving, so the mark lands in " +
      "the slot the message occupied instead of the list closing over it — and what it paints SAYS SO",
      { screen: seen(w), text: row.textContent });
    ok(row.textContent.indexOf("body-of-$a") < 0 && !/\bsecond\b/.test(row.textContent),
      "D: and it does not paint the body it replaced", row.textContent);
  }

  // The message is in a tier NOT on screen.
  {
    const w = W.world();
    w.raw("$a", "!u:hs", 100); w.raw("$s1", "!s:hs", 200); w.raw("$s2", "!s:hs", 201);
    click(w, "Everyone");
    const before = seen(w).join();
    w.redaction("$s1", "!s:hs");
    ok(rec(w, "!s:hs", "$s1").redacted === true && rec(w, "!s:hs", "$s2").redacted === false,
      "D: A REDACTION ROUTES TO THE CHANNEL THE MESSAGE IS IN, not the tier being viewed. Buffers are " +
      "per channel and a redaction carries its own room_id — driven rather than assumed, because " +
      "there is no reason a redaction must route the way a message does", [rec(w, "!s:hs", "$s1"), rec(w, "!s:hs", "$s2")]);
    ok(rec(w, "!u:hs", "$a").redacted === false,
      "D control: and the VISIBLE tier is untouched, so the routing is a routing rather than a " +
      "transition applied to whichever buffer was to hand", rec(w, "!u:hs", "$a"));
    ok(seen(w).join() === before && !w.box.children.some((n) => /Message deleted/.test(n.textContent)),
      "D: and nothing on screen changes, because the tombstone is in a tier that is not on screen", seen(w));
    click(w, "Staff");
    ok(seen(w).join() === "m:$s1,m:$s2" && /Message deleted/.test(w.box.children[0].textContent),
      ...
      "whenever the person next looks, not only if they were looking at the time", seen(w));
  }

  // A REDACTED UNDECRYPTABLE ROW MUST NOT BECOME A TOMBSTONE ON SCREEN.
  // `mutate-j11-redact` M4 survived before this block existed: the buffer keeping `failed` was driven,
  // and the renderer's choice between hiding the row and painting a mark was not. Two decisions.
  {
    const w = W.world();
    w.raw("$f", "!u:hs", 100, "unreadable", "@a:hs", true); w.raw("$g", "!u:hs", 101);
    click(w, "Everyone");
    ok(rec(w, "!u:hs", "$f") && rec(w, "!u:hs", "$f").failed === true && seen(w).join() === "m:$g",
      "D hidden-row: APPLIED — the undecryptable message is HELD and not drawn", { held: rec(w, "!u:hs", "$f"), screen: seen(w) });
    w.redaction("$f", "!u:hs");
    ok(seen(w).join() === "m:$g" && rec(w, "!u:hs", "$f").redacted === true && rec(w, "!u:hs", "$f").failed === true,
      "D: a redaction of a row this device could NOT DECRYPT paints nothing. The row was hidden, and a " +
      "deletion must not reveal that an unreadable message existed — information the person did not have " +
      "before", { screen: seen(w), rec: rec(w, "!u:hs", "$f") });
  }
  ...
  // A message this client never held.
  {
    ...
    click(w, "Everyone");
    let threw = null;
    try { w.redaction("$never", "!u:hs"); } catch (e) { threw = e.message; }
    ok(!threw && w.box._chat.bufs["!u:hs"].size() === 1 && seen(w).join() === "m:$a",
      "D: A REDACTION FOR A MESSAGE THIS CLIENT NEVER HELD IS A NO-OP, NOT AN ERROR — and it is the " +
      "NORMAL case rather than the exotic one: buffers are capped and per channel, the client may have " +
      ...
      "annotate one", { threw, screen: seen(w) });
  }

  // A redaction from a READABLE channel that is no tier and holds no buffer.
  {
    const w = W.world();
    w.raw("$a", "!u:hs", 100);
    click(w, "Everyone");
    w.sb.Chat.setReadableTiers(["!u:hs", "!s:hs", "!orphan:hs"]);   // passes the door
    let threw = null;
    try { w.redaction("$a", "!orphan:hs"); } catch (e) { threw = e.message; }
    w.CP.renderChat();                                               // repaint before reading
    ok(!threw && rec(w, "!u:hs", "$a").redacted === false && !/Message deleted/.test(w.box.children[0].textContent),
      "D: A REDACTION FROM A CHANNEL THAT HOLDS NO BUFFER HERE DELETES NOTHING — not even a message of the " +
      "same id in the tier on screen. This row only asserted 'does not throw' until ddjp_425, and the tree " +
      "then routed an unrecognised channel to the VISIBLE tier and tombstoned it", { threw, rec: rec(w, "!u:hs", "$a") });
    ok(!w.box._chat.bufs["!orphan:hs"],
      "D: and it does not CREATE a buffer for that channel — a redaction is never how a channel is admitted",
      Object.keys(w.box._chat.bufs));
  }
}
...
// 403 drift `main/10-capabilities.md` exists to prevent. J15's DM send is the precedent.
{
  ...
  const plm = mbSrc.match(/function _powerLevels\(sendLevel, creatorId, isSpace(?:, gates)?\)[\s\S]*?\n  \}/);
...
// to assume a round trip happened.
{
  ...
  const plm = src.match(/function _powerLevels\(sendLevel, creatorId, isSpace(?:, gates)?\)[\s\S]*?\n  \}/);
  ...
  function drive(fnSrc, name, args) {
    ...
    // `_chatRow` wires the sender's name to the user card, which moved to ui/roster.js at
    // ddjp_391. The stub records nothing — this guard's subject is what a redaction leaves
    // VISIBLE, not who the card belongs to — but it has to exist or the row dies before the
    // assertion, and a panel that died is not a panel that showed the wrong thing.
    ctx.Roster = { _wireCardTrigger: () => {} };
    ctx.globalThis = ctx;
    vm.createContext(ctx);
...
// ═══ the harness's own gate, both directions ═════════════════════════════════════════════════
{
  ...
  // The removal rows left with `driveRemoval` at ddjp_425 (PART D drives the real file now, and its
  // premises are APPLIED checks in each block, the never-held no-op among them). What is left is the
  // ROW reading, in both directions.
  ok(refusals === 1 && admits === 1,
    "the admissibility gate refuses a broken row reading and ADMITS a sound one", rows);
}
