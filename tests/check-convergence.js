...
const DEFAULT_SETTINGS = JSON.parse(JSON.stringify(makeClient().getState().settings));   // reducer default (drift-proof)
const EXPECTED = {
  // nowPlaying now carries a snapshot of the settings IN FORCE when the song started
  // (Phase 2 log-ordered settings). This scenario has no settings events, so the
  // snapshot is the default settings. deepStrictEqual checks it too.
  // ...and WHICH settings event those frozen values came from. This scenario has no settings
  // events at all, so the answer is null — a room still on defaults references nothing, and that
  // is verifiable by anyone, because defaults are code rather than data.
  nowPlaying: { dj: "@c:hs", song: { videoId: "S3", videoUrl: null }, pi: "$08", startedAt: 420000, skipped: false,
    settings: DEFAULT_SETTINGS, settingsFrom: null },
...
try {
  const _g = JSON.parse(j(got));
  assert.deepStrictEqual(_g, EXPECTED);
} catch (e) {
...
// 8) ddjp.dj.undeclare — a DJ removes ONE song from their OWN declared buffer
// WITHOUT playing it (the room-queue-section remove/swap primitive, 14 §4).
// Sender-only, pure, total, and must converge like every other event.
//
// 8a) Straight removal. A joins S1 and declares S2 (buffer [S1,S2]), then
// undeclares S1 -> [S2]; the genesis play starts S2 (S1 is gone, NEVER played),
// and S2 was A's last song so A falls out after it, leaving [B].
const UNDECL = [
  raw("$u1", 1, "@a:hs", { t: "ddjp.dj.join",      v: "S1" }),
  raw("$u2", 2, "@a:hs", { t: "ddjp.dj.declare",   v: "S2" }),
  raw("$u3", 3, "@a:hs", { t: "ddjp.dj.undeclare", v: "S1" }),   // pull S1 back off -> [S2]
  raw("$u4", 4, "@b:hs", { t: "ddjp.dj.join",      v: "S3" }),   // B behind A
  raw("$u5", 5, "@a:hs", { t: "ddjp.dj.play",      p: null }),   // genesis: A plays S2 (not S1), runs out -> [B]
];
const undeclBase = makeClient();
UNDECL.forEach((e) => undeclBase.ingest(e));
{
  const np = undeclBase.getState().nowPlaying;
  if (!np || !np.song || np.song.videoId !== "S2")
    fail("ddjp.dj.undeclare: undeclared S1 must be gone -> genesis play starts S2", "nowPlaying=" + j(np));
  const rot = undeclBase.getState().rotation.map(r => r.user);
  if (j(rot) !== j(["@b:hs"]))
    fail("ddjp.dj.undeclare: A runs out after S2 and falls out, leaving [B]", j(rot));
}
const undeclCanonical = j(undeclBase.getState());
for (let i = 0; i < 200; i++) {
  const C = makeClient();
  shuffle(UNDECL).forEach((e) => C.ingest(e));
  if (j(C.getState()) !== undeclCanonical) fail("ddjp.dj.undeclare: a shuffled order diverged", "run " + i);
}

// 8b) Race safety: if the song already PLAYED, a later undeclare of it is a clean
// no-op. A buffers [S1,S2], plays S1 (now-playing), then undeclares S1 -> the
// buffer stays [S2] and now-playing is untouched. Converges across arrival orders
// because the (l, event_id) sort is identical on every client.
const UNDECL_RACE = [
  raw("$r1", 1, "@a:hs", { t: "ddjp.dj.join",      v: "S1" }),
  raw("$r2", 2, "@a:hs", { t: "ddjp.dj.declare",   v: "S2" }),
  raw("$r3", 3, "@a:hs", { t: "ddjp.dj.play",      p: null }),   // A plays S1 -> now-playing S1, buffer [S2]
  raw("$r4", 4, "@a:hs", { t: "ddjp.dj.undeclare", v: "S1" }),   // S1 already played -> NO-OP
];
const raceBase = makeClient();
UNDECL_RACE.forEach((e) => raceBase.ingest(e));
{
  const np = raceBase.getState().nowPlaying;
  if (!np || np.song.videoId !== "S1") fail("undeclare race: now-playing S1 must be untouched", "np=" + j(np));
  const buf = (raceBase.getState().rotation.find(r => r.user === "@a:hs") || {}).pending || [];
  if (j(buf.map(s => s.videoId)) !== j(["S2"])) fail("undeclare race: buffer should still be [S2]", j(buf.map(s => s.videoId)));
}
const raceCanonical = j(raceBase.getState());
for (let i = 0; i < 200; i++) {
  const C = makeClient();
  shuffle(UNDECL_RACE).forEach((e) => C.ingest(e));
  if (j(C.getState()) !== raceCanonical) fail("ddjp.dj.undeclare race: a shuffled order diverged", "run " + i);
}

// 8c) Totality: undeclare by a non-member, with a missing/non-string v, or of an
// unknown id are all clean no-ops that never crash or move state; undeclaring the
// ONLY buffered song empties the buffer and drops the member (the fall-out rule).
const undeclT = makeClient();
undeclT.ingest(raw("$t0", 1, "@x:hs", { t: "ddjp.dj.undeclare", v: "Nope" }));     // non-member -> no-op
undeclT.ingest(raw("$t1", 2, "@a:hs", { t: "ddjp.dj.join",      v: "S1" }));
undeclT.ingest(raw("$t2", 3, "@a:hs", { t: "ddjp.dj.undeclare" }));                // missing v -> no-op
undeclT.ingest(raw("$t3", 4, "@a:hs", { t: "ddjp.dj.undeclare", v: 123 }));        // non-string v -> no-op
undeclT.ingest(raw("$t4", 5, "@a:hs", { t: "ddjp.dj.undeclare", v: "Other" }));    // unknown id -> no-op
{
  const buf = (undeclT.getState().rotation.find(r => r.user === "@a:hs") || {}).pending || [];
  if (j(buf.map(s => s.videoId)) !== j(["S1"])) fail("undeclare totality: no-ops must leave [S1] intact", j(buf.map(s => s.videoId)));
}
undeclT.ingest(raw("$t5", 6, "@a:hs", { t: "ddjp.dj.undeclare", v: "S1" }));       // last song -> empty -> drop member
if (undeclT.getState().rotation.length !== 0)
  fail("undeclare totality: removing the only buffered song must drop the member (fall-out rule)", j(undeclT.getState().rotation));

console.log("[convergence] PASS — rotation, advance lock, fall-out, move/remove/reset, dj.order, dj.undeclare, and room.settings (chat + bg) all converged across " + runs + " shuffled orders");
process.exit(0);
