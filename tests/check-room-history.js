...
// SUBJECT: backends/backend1/statederiver.js
...
const { StateDeriver } = loadInContext(["backends/backend1/ranks.js", "backends/backend1/statederiver.js"], {});
...
let _l = 0;
function join(user, vid, ts) { return { eventId: "j" + (++_l), l: _l, ts: ts || _l * 60000, senderRank: 20, sender: user, type: "ddjp.dj.join", content: { v: vid } }; }
function play(user, p, ts) { return { eventId: "p" + (++_l), l: _l, ts: ts || _l * 60000, senderRank: 20, sender: user, type: "ddjp.dj.play", content: { p: p || null } }; }
...
(function () {
  ...
  const A = "@a:x", B = "@b:x";
  const e = [
    join(A, "aaaaaaaaaaa", 60000),
    join(B, "bbbbbbbbbbb", 60060),
  ];
  let st = StateDeriver.derive(e);
  eq(st.history, [], "no plays yet -> empty history");

  const p1 = play(A, null, 120000);                 // genesis play: starts A's song
  e.push(p1);
  ...
  eq(st.history[0].at, 120000, "history entry carries the play's ts (for time-ago)");
  ok(st.history[0].pi === p1.eventId, "history entry carries the play-instance id");

  e.push(play(B, p1.eventId, 180000));              // advance: starts B's song
  st = StateDeriver.derive(e);
...
(function () {
  _l = 0;
  const A = "@a:x";
  const p1 = play(A, null, 120000);
  const e = [join(A, "aaaaaaaaaaa", 60000), p1, play(A, "wrong-pi", 150000)];
...
(function () {
  _l = 0;
  const A = "@a:x", B = "@b:x";
  const j1 = join(A, "aaaaaaaaaaa", 60000);
  const j2 = join(B, "bbbbbbbbbbb", 60060);
  const p1 = play(A, null, 120000);
  const p2 = play(B, p1.eventId, 180000);
