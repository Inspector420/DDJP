...
// WRONG IN BOTH DIRECTIONS BEFORE `ddjp_384`, and it is the row that shows a bucket-3 declaration can
// be. `streammanager.js` was MISSING though it is asserted on directly and repeatedly —
// `seedValidation()` is read and judged at six sites (PART A's frozen `at`, PART B's moved `at`) and
// `seedLicensesForget()` is asserted `=== true` twice. The verdict-per-SIGNATURE throttle that PARTs
// A and B both rest on is StreamManager's record, not the floor's.
//
// `checkpointformat.js` was DECLARED and is a vehicle. It appears twice outside the load list, both
// inside fixture builders — `sealed()` computes a valid fingerprint so a checkpoint is well-formed,
// `fileOf()` builds a save file to feed in — which is the same use `StateDeriver.buildSeed` and
// `defaultSettings` get in this file WITHOUT being declared, so the file was already inconsistent
// with itself. Its one other reach is `next.h !== cp.h`, labelled `B: CONTROL` by the author to stop
// the real assertion holding for the wrong reason. **A module appearing only in fixture builders and
// one setup control is a vehicle** — the same call settled for `floor.js` in
// `check-settings-readback` at `ddjp_384`, and declaring it here would contradict that ruling.
//
