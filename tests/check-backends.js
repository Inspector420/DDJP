...
// `08-build-and-deploy.md` and it applies here too.
{
  ...
  // BOTH PRESENT, THEN ORDERED. `indexOf` returns -1 when absent and -1 is less than any index, so
  // deleting the resolve entirely would have satisfied this comparison. The same weakness shipped
  // twice in this file; it is worth reading every `a.indexOf(x) < b.indexOf(y)` with that in mind.
  ok(joinBody.indexOf("Backends.resolveMode") >= 0 &&
     joinBody.indexOf("_initModules(current") >= 0 &&
     joinBody.indexOf("Backends.resolveMode") < joinBody.indexOf("_initModules(current"),
...
// ── PART U — WHEN THE RUNNER SEALS: THE SHARED ROOM'S RULE, DRIVEN THROUGH THE REAL BEAT (J61) ─
// The only check on this decision used to be a regex proving `checkpointEvery` and
// `checkpointCooldownMs` were SPELLED in runner.js, and J39's backend2 sweep showed what that bought:
// every comparison in the decision survived, including the one that made it AND where the shared
// room's gate is OR. So this part starts a REAL runner and drives it only through the interval it
// registers as its beat — the production caller — against the real backend1 `Checkpoint` and
// `Vouch` (what counts, from its one home), the real fingerprint, and the real `B2Checkpoint`.
//
// THE LOCAL CLOCK IS FROZEN far from every server stamp here. So each row that turns on time turns
// on the SERVER stamps alone: a decision reading the tab's clock could not tell these rows apart.
{
  const intervals = {};
  const ctx = { console, Date: { now: () => 5e12 },
    setInterval: (fn, ms) => { intervals[ms] = fn; return ms; }, clearInterval: () => {} };
  vm.createContext(ctx);
  ctx.Logger = { info() {}, warn() {}, debug() {}, error() {} };
  ctx.Backends = { register() {}, active: () => "backend2", resolveMode: (c) => (c && c.ddjp_mode) || null };
  for (const f of ["backend1/ranks.js", "backend1/consensushash.js", "backend1/trustpolicy.js",
                   "backend1/checkpointformat.js", "backend1/dials.js", "backend1/vouch.js",
                   "backend1/checkpoint.js", "backend2/checkpoint.js", "backend2/authority.js", "backend2/runner.js"]) {
    vm.runInContext(fs.readFileSync(path.join(ROOT, "backends/" + f), "utf8"), ctx, { filename: f });
  }
  vm.runInContext("globalThis.R = B2Runner; globalThis.CP = B2Checkpoint; globalThis.C1 = Checkpoint;", ctx);
  const S = { log: [], legal: {}, settings: { checkpointEvery: 5, checkpointCooldownMs: 600000 } };
  ctx.StreamManager = { getState: () => ({ settings: S.settings, marker: 1 }),
                        getLog: () => S.log.slice(), isLegal: (id) => S.legal[id] !== false };
  const sent = [];
  ctx.MatrixBridge = { mayAuthor: () => ({ ok: true }), onAuthorReady: (fn) => fn(), getMyPowerLevel: () => 99,
    getCreateContent: () => ({ ddjp_mode: "backend2" }), onRawEvent() {}, offRawEvent() {},
    sendEvent: (room, type, c) => { sent.push({ room, type, c }); return "$ok"; } };
  const CH = { events_uncategorized: "!intents:hs", events_owner: "!log:hs" };
  const { R, CP } = ctx;
  ok(R.start({ channels: CH }).ok === true, "U: APPLIED — the runner starts, or nothing below is exercised");
  ok(typeof intervals[60000] === "function", "U: APPLIED — and registers its beat, which is the only caller driven here");
  ok(typeof R.sealDue === "function", "U: the decision is one exported function, so this drives the real rule");

  let nextL = 1;
  const ev = (t, ts, legal) => { const id = "$e" + nextL;
    S.log.push({ eventId: id, type: t, content: { t: t }, l: nextL++, ts: ts, sender: "@a:hs" });
    if (legal === false) S.legal[id] = false; return id; };
  const seals = () => sent.filter((x) => x.type === "ddjp.checkpoint");
  const beat = () => { const n = seals().length; intervals[60000](); return seals().length - n; };
  const why = () => { const v = R._maybeSeal(); return v && v.reason; };   // read-only while not due
  // The checkpoint coming back, adopted at a SERVER stamp exactly as the door hands it over.
  const echo = (serverTs) => { const cp = seals()[seals().length - 1].c;
    return CP.observe(cp, { id: "$cp" + seals().length, at: serverTs }); };
  const COOL = S.settings.checkpointCooldownMs, T0 = 1000000000;

  // U1 — nothing at all: no seal, and it says why.
  ok(beat() === 0 && why() === "nothing-changed", "U1: an empty log never seals", why());
  // U2 — no floor yet and one countable act: overdue by definition, as in the shared room.
  ev("ddjp.dj.join", T0);
  ok(beat() === 1, "U2: with no floor, one countable event makes a seal due", seals().length);
  ok(echo(T0).adopted === true && CP.floorAt() === T0, "U2: APPLIED — adopted at its server stamp", CP.floorAt());

  // U3 — THE J61 DEFECT: a quiet room that changed, on the TIME trigger alone.
  ev("ddjp.dj.play", T0 + COOL - 1);
  ok(beat() === 0 && why() === "not-due",
    "U3: one tick under the cooldown by the SERVER clock is not due", why());
  ev("ddjp.dj.skip", T0 + COOL);
  ok(beat() === 1,
    "U3: A QUIET ROOM SEALS ON TIME — two countable events, far below checkpointEvery, and the cooldown " +
    "reached by the server stamps. Requiring the count AND the cooldown is what made a quiet bot room " +
    "never seal; the local clock did not move between these two beats, so this is the server's", seals().length);
  echo(T0 + COOL);

  // U4 — a busy room INSIDE its cooldown, on the COUNT trigger alone.
  const B = T0 + COOL;
  for (let i = 1; i <= 4; i++) ev("ddjp.dj.join", B + i * 1000);
  ok(beat() === 0 && why() === "not-due", "U4: one event short of checkpointEvery is not due", why());
  ev("ddjp.dj.join", B + 5000);
  ok(beat() === 1, "U4: A BUSY ROOM SEALS ON COUNT — checkpointEvery countable events, well inside the " +
    "cooldown. Either trigger is enough, as the shared room's gate says", seals().length);
  // The next beat arrives BEFORE that checkpoint comes back. The floor has not moved, so counted from
  // the floor alone the same five events are still "due" — and the room would seal them twice.
  ok(beat() === 0 && why() === "nothing-changed",
    "U4: COUNTED FROM THE HEAD IT LAST SEALED, not only from the floor — so a beat between sending a count " +
    "seal and adopting it does not seal the same events again", why());
  echo(B + 5000);

  // U8 — the event AT the floor's position is banked, not new (J39's runner:280 row).
  for (let i = 1; i <= 4; i++) ev("ddjp.dj.join", B + 5000 + i);
  ok(beat() === 0 && R._maybeSeal().newEvents === 4,
    "U8: only events ABOVE the floor count — the head the floor banked is not a fifth", R._maybeSeal());
  ev("ddjp.dj.join", B + 6000);
  ok(beat() === 1, "U8: APPLIED — and the fifth above it is", seals().length);
  echo(B + 6000);

  // U5 — NOTHING COUNTABLE CHANGED: minor acts only, long past the cooldown. The list is backend1's own.
  const F5 = B + 6000;
  for (const t of ["ddjp.dj.vote", "ddjp.dj.save", "ddjp.dj.vote", "ddjp.media.len", "ddjp.dj.save", "ddjp.dj.vote"]) ev(t, F5 + 10 * COOL);
  ok(ctx.C1._countable(S.log.slice(-6)) === 0, "U5: PREMISE — the shared room counts none of these", ctx.C1._countable(S.log.slice(-6)));
  ok(beat() === 0 && why() === "nothing-changed",
    "U5: votes, saves and display lengths are not a reason to seal, however long it has been — the " +
    "shared room's countable rule, read from its one home and not copied", why());
  ev("ddjp.dj.play", F5 + 10 * COOL);
  ok(beat() === 1, "U5: CONTROL — one countable act on the same state makes it due", seals().length);
  echo(F5 + 10 * COOL);

  // U6 — a REFUSED act changed nothing, so it does not count either.
  const F6 = F5 + 10 * COOL;
  const refused = [];
  for (let i = 1; i <= 6; i++) refused.push(ev("ddjp.dj.join", F6 + i, false));
  ok(beat() === 0 && why() === "nothing-changed", "U6: acts the fold refused are not counted", why());
  for (const id of refused) S.legal[id] = true;
  ok(beat() === 1, "U6: CONTROL — the same six, legal, are a count seal", seals().length);
  echo(F6 + 6);

  // U7 — A SEAL IN FLIGHT HOLDS THE CLOCK TRIGGER, and adopting it releases the hold.
  const F7 = F6 + 6;
  ev("ddjp.dj.play", F7 + COOL);
  ok(beat() === 1, "U7: APPLIED — a time seal is sent", seals().length);
  ev("ddjp.dj.skip", F7 + COOL + 1);
  ok(beat() === 0 && why() === "in-flight",
    "U7: until that checkpoint comes back the floor has not moved, and the room must not seal the same " +
    "overdue history twice", why());
  echo(F7 + COOL + 1);
  ok(beat() === 0 && why() === "not-due", "U7: once it is adopted, the clock restarts from the new floor", why());

  // U9 — A RELOAD. The runner's memory resets; the floor's server stamp does not.
  R.stop(); ok(R.start({ channels: CH }).ok === true, "U9: APPLIED — restarted");
  const F9 = F7 + COOL + 1;
  ev("ddjp.dj.join", F9 + COOL - 1);
  ok(beat() === 0 && why() === "not-due", "U9: after a reload the cooldown is still measured from the floor", why());
  ev("ddjp.dj.join", F9 + COOL);
  ok(beat() === 1, "U9: and comes due on the server's clock exactly as before the reload", seals().length);
  R.stop();
}
