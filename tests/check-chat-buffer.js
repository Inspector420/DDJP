...
// `tooLong` joined at ddjp_430 for the same reason `redacted` did: it is STATE about the message — it
// arrived over the 512-character limit — and like a tombstone it holds no content. Still no kind/src:
// how a body DISPLAYS is decided at render time, never stored.
eq(Object.keys(rec0).sort(), ["body", "failed", "id", "redacted", "sender", "tooLong", "ts"],
  "record holds {id,sender,body,failed,redacted,tooLong,ts} — content + state + order key, no kind/src");
ok(rec0.redacted === false,
  "and an ordinary message is not redacted, so the flag is a reading rather than a constant");
ok(rec0.tooLong === false,
  "and it is not over the limit either — the same: a reading, not a constant");
