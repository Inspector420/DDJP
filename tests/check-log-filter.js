...
// SUBJECT: ui/panels.js, core/logger.js
// The log filter moved to ui/panels.js with the LOGS PANEL cluster at ddjp_387. Its PREMISE —
// that the region is extractable — fired on the move rather than passing on an empty slice,
// which is why this is a re-point and not a repair.
...
// `buildMainDom` stayed in ui/interface.js when the panels cluster left, and one row below reads
// it rather than the cluster. Named here so the two reads cannot be confused for one.
const SHELL_SRC = fs.readFileSync(path.join(ROOT, "ui/shell.js"), "utf8");
...
function panel(opts) {
  ...
  `;
  // `H` is the interim host seam (ui/base.js §host): since ddjp_387 the extracted filter reaches
  // `el` and `clear` through it rather than as bare names. It is built from the SAME two values
  // passed below, not from a second pair — one harness, one definition of what `clear` does.
  const _el = el;
  const _clear = (n) => { n.children.length = 0; };
  const make = new Function("refs", "el", "clear", "H", "_priorLog", "_sessionLog", "_LOG_LEVELS",
...
// ═══ PART F — A REQUEST VERDICT IS REPORTED, BOTH WAYS ═══════════════════════════════════════
{
  ...
  settle().then(() => {
    ...
    // THIS ROW READS A DIFFERENT FILE FROM THE REST OF THIS GUARD, ON PURPOSE. The filter moved to
    // ui/panels.js at ddjp_387; this button's handler is built inline inside `buildMainDom`, which
    // did not — it stays in ui/interface.js and is bound for ui/shell.js. Pointing this row at
    // UI_SRC would have it scanning the wrong file and passing or failing for the wrong reason.
    ok(/BotRuntime\.report\(/.test(SHELL_SRC),
      "G: WIRED — the file holding `buildMainDom` calls `report()`, so it is not another " +
      "loaded-and-never-" +
