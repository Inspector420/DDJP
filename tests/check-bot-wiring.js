...
// SUBJECT: features/botruntime.js, features/room.js, ui/player.js
// PART H reads the UI's media load site, and this line never said so. It named the file it
// loads rather than the files it makes assertions ABOUT, which is the forty-third signature.
...
// guard, is a fix the next session can undo without anything going red.
{
  ...
  // SLICED TO THE END OF THE FUNCTION, NOT TO A CHARACTER COUNT. This read `plIdx + 1600`, and the
  // line it pins sat at 1663 the moment `_powerLevels` grew a parameter — so the guard went red for
  // a reason with nothing to do with `m.space.child`. A window measured in characters is a window
  // that expires; the next declaration is where the function actually ends.
  const plIdx = bridge.indexOf("function _powerLevels");
  const plEnd = bridge.indexOf("\n  function ", plIdx + 10);
  const plBody = bridge.slice(plIdx, plEnd > plIdx ? plEnd : plIdx + 4000);
...
// removed it. A rule with no gate is not built.
{
  ...
  const uiSrc = rd("ui/player.js");        // the media load site, moved at ddjp_386
