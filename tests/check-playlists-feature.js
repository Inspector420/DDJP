...
function fresh() {
  ...
  const sb = loadInContext(["core/playlistdoc.js", "features/playlists.js"], { Store: store, UserQueue: uq, Date, URL });
...
(async () => {
  ...
  // ---- addTrackByUrl: link/bare-id -> videoId, then routes through addTrack ----
  {
    const { P, store } = fresh();
    P.init(); store._resolveNext(); await settle();
    const a = await P.create("Links");
    ok((await P.addTrackByUrl(a.id, "https://www.youtube.com/watch?v=" + V1)).ok, "addTrackByUrl accepts a watch link");
    ok((await P.addTrackByUrl(a.id, "https://youtu.be/" + V2)).ok, "addTrackByUrl accepts a youtu.be link");
    ok((await P.addTrackByUrl(a.id, V3)).ok, "addTrackByUrl accepts a bare id");
    const dupe = await P.addTrackByUrl(a.id, "https://www.youtube.com/watch?v=" + V1);
    ok(!dupe.ok && /already/.test(dupe.reason), "addTrackByUrl dedups via addTrack (same id)");
    const bad = await P.addTrackByUrl(a.id, "https://example.com/nope");
    ok(!bad.ok && /link/.test(bad.reason), "addTrackByUrl rejects a non-YouTube link");
    ok(!(await P.addTrackByUrl(a.id, "")).ok, "addTrackByUrl rejects empty input");
    ok(!(await P.addTrackByUrl("no-such-playlist", "https://youtu.be/" + V1)).ok, "addTrackByUrl rejects a missing playlist (via addTrack)");
  }
