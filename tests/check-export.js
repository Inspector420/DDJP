...
// SUBJECT: ui/shell.js, ui/settings.js, index.html, backends/backend1/floor.js, backends/backend1/streammanager.js
// `streammanager.js` ADDED at `ddjp_384` — under-declared, and the header below already explains why
// it is the accused: `CheckpointFormat` is a walled backend internal that `ui/` cannot reach, so a
// guard calling `saveFile` directly would pass on a build where the seam does not exist. The backend
// half is therefore driven THROUGH `StreamManager` — "the interface the app actually has" —
// `exportCheckpoint` and `heldCheckpoints` at eight sites, including a direct verdict assertion that
// an unheld hash answers `not-held`.
// `checkpointformat.js` is deliberately NOT a subject: the header says this guard goes "never through
// the format module", and `readFile` appears only to DECODE what StreamManager produced, which is a
// measuring instrument. `floor.js` STAYS — PART A judges its API surface directly (the accessor
// exists, the `_seenForTest` seam is gone rather than kept alongside) and its copy rule.
//
...
{
  // PART F asks whether the export section is reachable from the settings panel, and that panel
  // moved to ui/settings.js at ddjp_393 while `renderExportSection` stayed in SCREENS. The
  // property spans both files, so both are read rather than one being swapped for the other.
  const ui = ["ui/base.js", "ui/shell.js", "ui/settings.js", "ui/screens.js"].map(code).join("\n;\n");
  ...
  // `renderSettings` is in ui/settings.js since ddjp_393. The brace-counting below is a fact about
  // the file that HOLDS the function, so this reads that file rather than the one it used to be in.
  const rawLines = fs.readFileSync(path.join(ROOT, "ui/settings.js"), "utf8").split("\n");
