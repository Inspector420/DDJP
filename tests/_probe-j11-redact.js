...
function driveRemoval(opts) {
  ...
  // `_chatRow` wires the sender name to the user card, which moved to ui/roster.js at ddjp_391.
  // A stub, because this probe's subject is what a redaction leaves visible rather than who the
  // card belongs to — but it must EXIST or the row dies before the assertion, and a row that
  // died is not a row that showed the wrong thing.
