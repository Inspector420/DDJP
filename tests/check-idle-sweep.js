...
// SUBJECT: features/botruntime.js, features/room.js
...
// exclusion left the whole suite green.
{
  ...
  // `_channels()` is the accessor the transport reads since J24 slice 1 made the table per-engine;
  // `CHANNELS` is kept beside it so an older slice of the source still resolves. Both answer with
  // `rows`, which came from the transport itself — so the key stays the tree's, not this file's.
  const sandbox = { console, CHANNELS: rows, _channels: () => rows, _rankFromKey: (k) => 0 };
  ...
  function chatTree(sameRoom, recordWhileStopped, chatBeforeStop) {
    ...
    const sb = loadInContext([
      ...
      MatrixBridge: { getUserId: () => "@bot:hs", getMyRank: () => 99, getMyPowerLevel: () => 99,
        ...
        invitedMembersOf: () => [],
        // AN OBJECT WITH `.ok`, like the real one. A bare boolean here made the sweep read
        // `v.ok` on a primitive and refuse — the fixture disagreeing with production about the
        // SHAPE of an answer, which is the same class as the map this part exists to catch.
        mayAuthor: () => ({ ok: true }) },
      ServerClock: { serverNow: () => NOW },
