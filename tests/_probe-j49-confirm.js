...
//                    tree that is exactly one function, defined in `ui/player.js`, and what it
...
// property of `ui/player.js`, so it is DRIVEN here rather than read: R1 runs the shipped
...
const vm = require("vm");
const { loadInContext, makeBackendsStub, ROOT } = require("./_load");
const F = require("./_fixtures");

const PB_REL = "features/playback.js";
const UI_REL = "ui/player.js";   // the provider moved here from ui/interface.js at ddjp_386; re-pointed at ddjp_415
...
function driveDeclare(opts) {
  ...
  const sandbox = {
    ...
    Logger: _quiet,
    // `capabilities.js` REGISTERS AT LOAD through the backend seam (J24), so this hand-rolled context
    // needs the same stand-in the shared loader installs — taken from `_load.js`, exactly as
    // `_probe-j48-endon.js` does. Without it playback.js never loaded and every row refused, which
    // nothing noticed because no guard runs this probe (found at ddjp_415).
    Backends: makeBackendsStub(),
  };
  ...
  try {
    vm.runInContext(deps + "\n;\n" + source +
      "\n;globalThis.Playback = Playback; globalThis.Capabilities = B1Capabilities;",
...
// Every shape the YouTube iframe API can present, including the two its own comments name. MOVED
// here from `tools/probes/probe-j49-confirm.js` at ddjp_415 so the runner and the guard on the
// provider contract (`check-duration-provider`) drive one list rather than two copies.
const PLAYER_SHAPES = [
...
module.exports = {
  ...
  PB_REL, UI_REL, PROVIDER_ANCHOR, PLAYER_SHAPES,
};
