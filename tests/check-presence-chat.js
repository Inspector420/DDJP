...
// SUBJECT: backends/backend1/matrixbridge.js, features/roomupgrade.js
...
// pass, the two fighting forever with nothing reporting it.
{
  ...
  // THE REASON, NOT THE SPELLING. This pinned the exact text `CHANNELS.filter(...)` until J24
  // slice 1 made the table per-engine and the accessor `_channels()` replaced the constant. The
  // property was never the spelling: it is that the key is SELECTED BY KIND from the table and
  // never written as a literal. Both halves are now asserted directly, so the next rename of the
  // accessor cannot turn this red for a reason that has nothing to do with presence.
  ok(/\.filter\(\s*\(c\)\s*=>\s*c\.kind === "presence"\s*\)/.test(body),
    "E: derived by KIND from the channels table — the row is selected by its kind, whatever the "
    + "table accessor is called", body.slice(0, 200));
  ok(!/"presence_chat"/.test(body),
    "E: and NOT a literal. A literal `\"presence_chat\"` here would be the second copy, and a "
    + "renamed row would silently drop the exclusion", body.slice(0, 200));
  ...
  ok(/function _presenceChatKey/.test(src) && !/const _presenceChatKey\s*=/.test(src),
    "E: a function, so the lookup happens when CALLED — a const would read the table before it is "
...
{
  const src = fs.readFileSync(path.join(ROOT, "backends/backend1/matrixbridge.js"), "utf8");
  // EXTRACTED AGAINST THE REAL DERIVATION. Until J24 slice 1 this lifted a `const UPGRADE_BATCHES`
  // literal out of the source; the table is per-engine now and the batches are computed by
  // `_derived()` from whichever plan is bound. So the slice takes the table, the derivation and the
  // function under test, and pins `_channels()` to backend1's own rows — which is what an unbound
  // client resolves to anyway. This tests MORE than the old lift did: the grouping is now exercised
  // rather than copied.
  const table = src.slice(src.indexOf("const B1_CHANNELS = ["), src.indexOf("// Canonical string forms"));
  const derive = src.slice(src.indexOf("const _derivedCache = new WeakMap();"),
                           src.indexOf("function eventsKeyForLevel"));
  const batches = src.slice(src.indexOf("function UPGRADE_BATCHES()"),
                            src.indexOf("async function createUpgradeBatch"));
  const sandbox = { console };
  require("vm").createContext(sandbox);
  require("vm").runInContext(
    table + "\nfunction _channels() { return B1_CHANNELS; }\n" + derive + "\n" + batches
    + "\n;globalThis.H = highestPresentBatch; globalThis.C = B1_CHANNELS;", sandbox);
  ...
  // Accepts the accessor form too: J24 slice 1 made the batches per-engine, so `UPGRADE_BATCHES`
  // became `UPGRADE_BATCHES()`. The property is unchanged — the numbers come from the table's own
  // keys and are never listed — and pinning the punctuation would fail for a reason that has
...
// another, twice, because the batch NUMBER is what nothing else reads.
{
  ...
  // Same extraction shape as PART H, and the same reason (J24 slice 1): `maxUpgradeBatch` reads
  // `_derived()` now, so the derivation comes with it and `_channels()` is pinned to backend1's rows.
  const table = mb.slice(mb.indexOf("const B1_CHANNELS = ["), mb.indexOf("// Canonical string forms"));
  const derive = mb.slice(mb.indexOf("const _derivedCache = new WeakMap();"),
                          mb.indexOf("function eventsKeyForLevel"));
  const fn = mb.slice(mb.indexOf("function maxUpgradeBatch"), mb.indexOf("function highestPresentBatch"));
  const sandbox = {};
  require("vm").createContext(sandbox);
  require("vm").runInContext(
    table + "\nfunction _channels() { return B1_CHANNELS; }\n" + derive + "\n" + fn
    + ";globalThis.M = maxUpgradeBatch; globalThis.C = B1_CHANNELS;", sandbox);
