...
// SUBJECT: backends/backend1/matrixbridge.js, backends/backend1/matrixaccount.js, features/chat.js
...
// THE DM SCOPE LEFT THIS FILE AT J59 and lives in `matrixaccount.js` now. PART C is the better
// test for it: the two scopes are two files as well as two questions, and "leaving a room did not
// unbind the conversation" is now a claim ACROSS the seam rather than inside one closure.
const MA = sb.MatrixAccount;
...
ok(MA.inDMScope("!dm-them:hs"), "PART C premise: the DM scope did not bind");
...
ok(!MA.inDMScope("!ev-owner:hs"), "PART C: a Spine channel answers `inDMScope` TRUE");
...
ok(MA.inDMScope("!dm-them:hs"), "PART C: leaving a room unbound the DM scope. A conversation is " +
...
MA.setDMScope(["!dm-other:hs"]);
ok(MA.inDMScope("!dm-other:hs") && !MA.inDMScope("!dm-them:hs"),
...
// 7100, not 7000: since ddjp_442 a marker that does not MOVE writes nothing, and 7000 is the stamp this row
// already has — the premise below is "both rows reached the store", so the second touch must move it.
CP.dmTouch("!dm-other:hs", "@other:hs", 7100);
...
ok(pRows.badge === "DMs",
  "the DMs tab reads \"DMs\" with NO COUNT (owner ruling, ddjp_432). Its purple dot belongs to the LEVEL MODEL since " +
  "ddjp_441 — shown only while the DMs tab is not in front of you — and `check-chat-view` PART M drives it", pRows);
...
// THIS ROW READS A DIFFERENT FILE FROM THE REST OF THIS GUARD, ON PURPOSE. The DM panel is still
// in ui/interface.js (it comes out with ui/chat.js), but `_CARD_ACTIONS` left for ui/roster.js at
// ddjp_391 — the two halves of what used to be one banner span now live in two files, and this
// one line is where that shows. Same shape as check-log-filter's PART G.
const cardTable = P.extractNamed("ui/roster.js", "_CARD_ACTIONS");
