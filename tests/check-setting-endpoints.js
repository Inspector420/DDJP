...
// SUBJECT: backends/backend1/statederiver.js, features/queue.js, features/userqueue.js
...
// ── WHAT THIS GUARD DID NOT COVER UNTIL J62 — kept because the reasoning still holds ──────────
// SINCE `ddjp_418` THESE ARE COVERED: the skip-road and trust-table limits are `list` and `tiers`
// rows in SETTING_RANGES (PARTs L and T), and the event limits are the reducer's second table,
// `eventLimits()` (PART E). What follows is the record of why that was the fix.
// Driven: flipping `s.skipRoads.length >= 1` to `> 1` left this guard GREEN. That is not a hole
...
// THE FIFTH AND SIXTH KINDS (J62): a LIST (length bounds plus per-item bounds) and a TIER TABLE (the
// bounds on each row's `enough`). Counted into the partition like every kind before them.
const listKeys = keys.filter((k) => kindOf(k) === "list");
const tierKeys = keys.filter((k) => kindOf(k) === "tiers");
...
ok(rangeKeys.length + valueKeys.length + mapKeys.length + flagKeys.length + listKeys.length + tierKeys.length === keys.length,
  "C: every key in SETTING_RANGES must be classified as `range`, `values`, `map`, `flags`, `list` or `tiers` — a key in none is "
...
// ── PART L — LIST ENTRIES, every bound driven through the real merge (J62) ────────────────────
// Each road is built LEGAL except for the one number under test, so a refusal is that number's.
ok(listKeys.length >= 1, "L: PREMISE — the reducer declares at least one list entry", listKeys);
for (const key of listKeys) {
  const r = RANGES[key];
  const road = (g, v) => ({ guestPlus: g, vipPlus: v });
  const okRoad = road(Math.max(r.itemMin, 1), r.itemMin);          // legal: never both zero
  const got = (list) => apply({ [key]: list })[key];
  const same = (a, b) => JSON.stringify(a) === JSON.stringify(b);
  const listOf = (n) => Array.from({ length: n }, () => okRoad);
  ok(same(got(listOf(r.minItems)), listOf(r.minItems)), "L: `" + key + "` with exactly minItems (" + r.minItems + ") is ACCEPTED");
  ok(same(got(listOf(r.maxItems)), listOf(r.maxItems)), "L: `" + key + "` with exactly maxItems (" + r.maxItems + ") is ACCEPTED");
  ok(!same(got(listOf(r.maxItems + 1)), listOf(r.maxItems + 1)), "L: `" + key + "` with maxItems+1 is REFUSED whole");
  if (r.minItems > 0) ok(!same(got(listOf(r.minItems - 1)), listOf(r.minItems - 1)), "L: `" + key + "` with minItems-1 is REFUSED whole");
  for (const [label, rd, accept] of [
    ["guestPlus at itemMax", road(r.itemMax, 0), true], ["guestPlus at itemMax+1", road(r.itemMax + 1, 0), false],
    ["vipPlus at itemMax", road(0, r.itemMax), true],   ["vipPlus at itemMax+1", road(0, r.itemMax + 1), false],
    ["guestPlus at itemMin", road(r.itemMin, 1), true], ["guestPlus at itemMin-1", road(r.itemMin - 1, 1), false],
    ["vipPlus at itemMin", road(1, r.itemMin), true],   ["vipPlus at itemMin-1", road(1, r.itemMin - 1), false]]) {
    ok(same(got([rd]), [rd]) === accept, "L: `" + key + "` " + label + " must be " + (accept ? "ACCEPTED" : "REFUSED"), got([rd]));
  }
}

// ── PART T — TIER-TABLE ENTRIES, `enough` driven at both ends and one step out (J62) ───────────
ok(tierKeys.length >= 2, "T: PREMISE — both trust tables declare their bounds", tierKeys);
for (const key of tierKeys) {
  const r = RANGES[key];
  const rows = (e) => Array.from({ length: Ranks.TIER_COUNT }, () => (key === "vouchTable" ? { enough: e, always: false } : { enough: e }));
  const took = (e) => JSON.stringify(apply({ [key]: rows(e) })[key]) === JSON.stringify(rows(e));
  ok(took(r.enoughMin), "T: `" + key + "` enough = enoughMin (" + r.enoughMin + ") is ACCEPTED");
  ok(took(r.enoughMax), "T: `" + key + "` enough = enoughMax (" + r.enoughMax + ") is ACCEPTED");
  ok(!took(r.enoughMax + 1), "T: `" + key + "` enough = enoughMax+1 is REFUSED whole");
  ok(!took(r.enoughMin - 1), "T: `" + key + "` enough = enoughMin-1 is REFUSED whole");
  ok(took(null), "T: `" + key + "` enough = null (never) is ACCEPTED");
}
ok(JSON.stringify(RANGES.vouchTable) === JSON.stringify(RANGES.checkpointTable),
  "T: the two tables share ONE validator, so they must declare ONE set of bounds", [RANGES.vouchTable, RANGES.checkpointTable]);

// ── PART E — THE EVENT LIMITS, the reducer's second table, driven through the real fold (J62) ──
// Kept out of SETTING_RANGES by owner ruling: the panel reads that table, and these are not settings.
{
  const EL = StateDeriver.eventLimits ? StateDeriver.eventLimits() : null;
  ok(!!EL && typeof EL.songIdMaxLength === "number" && typeof EL.songUrlMaxLength === "number" && typeof EL.countMin === "number",
    "E: PREMISE — the reducer exports its event-limits table", EL);
  if (EL) {
    EL.songIdMaxLength = -1;
    ok(StateDeriver.eventLimits().songIdMaxLength !== -1, "E: eventLimits() hands out a COPY — a caller cannot move the reducer's limit");
    const lim = StateDeriver.eventLimits();
    let L = 0;
    const ev = (type, c, rank) => ({ eventId: "$" + (++L), event_id: "$" + L, type, sender: "@a:hs", senderRank: rank, l: L, ts: 1000 + L,
      content: Object.assign({ t: type }, c) });
    // a joined DJ and their pending songs live in `rotation`
    const pending = (c) => { L = 0; const st = StateDeriver.derive([ev("ddjp.dj.join", c, 40)]); const q = (st && st.rotation) || []; return (q[0] && q[0].pending && q[0].pending[0]) || null; };
    ok(!!pending({ v: "a".repeat(lim.songIdMaxLength) }), "E: a song id of exactly songIdMaxLength is ACCEPTED");
    ok(pending({ v: "a".repeat(lim.songIdMaxLength + 1) }) === null, "E: one character longer is REFUSED, never truncated");
    const atUrl = pending({ v: "abc", u: "h".repeat(lim.songUrlMaxLength) });
    ok(!!atUrl && atUrl.videoUrl === "h".repeat(lim.songUrlMaxLength), "E: a link of exactly songUrlMaxLength is KEPT", atUrl && atUrl.videoUrl && atUrl.videoUrl.length);
    const overUrl = pending({ v: "abc", u: "h".repeat(lim.songUrlMaxLength + 1) });
    ok(!!overUrl && overUrl.videoUrl === null, "E: one character longer is DROPPED and the song still stands", overUrl);
    const why = (n) => { L = 0; const r = StateDeriver.deriveRefusals([ev("ddjp.count.set", { id: "p1", k: "vote", n: n }, 99)]) || {}; return r["$1"] ? r["$1"].code : "accepted"; };
    ok(why(lim.countMin) === "accepted", "E: count.set with n = countMin is ACCEPTED", why(lim.countMin));
    ok(why(lim.countMin - 1) === "not-a-count", "E: count.set with n = countMin-1 is REFUSED as not-a-count", why(lim.countMin - 1));
  }
}

// ── PART S — THE NEW ENTRIES REACH THE PANEL FLAT AND AS A COPY (J62) ──────────────────────────
// The seam copies top-level fields and would pass a nested object BY REFERENCE (J07), which is why
// these entries are flat. Driven: the panel's copy carries the reducer's numbers, and mutating it
// leaves the reducer's table alone.
for (const key of listKeys.concat(tierKeys)) {
  const seen = (SM.settingRanges() || {})[key];
  ok(!!seen && JSON.stringify(seen) === JSON.stringify(RANGES[key]), "S: the panel's seam delivers `" + key + "` with the reducer's own numbers", seen);
  ok(Object.values(RANGES[key]).every((v) => typeof v === "number"), "S: `" + key + "` is FLAT — numbers only, nothing the seam would share by reference");
  if (seen) { for (const f of Object.keys(seen)) seen[f] = -999; }
  ok(Object.values(RANGES[key]).every((v) => v !== -999), "S: mutating the panel's copy of `" + key + "` does not move the reducer's limit");
}
// The panel half is ui/, which nothing here renders. What CAN be established is textual, and is
// labelled so: the two render functions read the table and no longer carry their own numbers.
{
  const ui = require("fs").readFileSync(path.join(__dirname, "..", "ui/settings.js"), "utf8");
  const body = (name) => { const a = ui.indexOf("function " + name + "("); if (a < 0) return ""; const b = ui.indexOf("\n  function ", a + 10); return ui.slice(a, b < 0 ? undefined : b).replace(/\/\/[^\n]*/g, ""); };
  for (const [fn, nums] of [["_renderTableSetting", ["50"]], ["_renderSkipRoadsSetting", ["200"]]]) {
    const b = body(fn);
    ok(b.length > 0 && /Room\.getSettingRanges\(\)\[key\]/.test(b), "S (textual): `" + fn + "` reads the reducer's table for its key");
    ok(nums.every((n) => !new RegExp("[\"'\\s(<>=]" + n + "\\b").test(b)), "S (textual): `" + fn + "` no longer spells its own copy of " + nums.join(", "));
  }
}
...
console.log("[setting-endpoints] PASS — every declared setting entry is driven through the endpoint "
  ...
  + "illegal at the leftmost value the panel offers, and the suite as shipped said nothing. Since J62 "
  + "the " + listKeys.length + " LIST and " + tierKeys.length + " TIER-TABLE entries are driven at every bound too, the "
  + "event limits are the reducer's second table and are driven through the real fold, and the panel "
  + "reads both list kinds through the seam as flat copies");
