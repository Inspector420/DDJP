...
// SUBJECT: features/chat.js, ui/shell.js, backends/backend1/matrixaccount.js, backends/backend1/matrixbridge.js
...
function makeChat(opts) {
  ...
  const sb = loadInContext(["core/logger.js", "features/chat.js"], {
    ...
    // ── TWO STUBS SINCE J59, BECAUSE THERE ARE TWO MODULES ────────────────────────────────
    // The DM surface left `matrixbridge.js` for `matrixaccount.js`. Injecting one object under
    // both names would have kept this green while saying nothing about WHICH module chat.js
    // reaches — and the seam is half of what this guard is about. Split, so a call landing on
    // the wrong module is `undefined` here rather than silently satisfied.
    MatrixBridge: {
    ...
    MatrixAccount: {
      ...
      //
      // AND UNTIL J59 IT WAS STUBBED ANYWAY, BY A DUPLICATE KEY. `findDMRoom: () => null` sat
      // thirteen lines below this one in the same object literal and WON — so the comment above
      // described a mechanism the object did not have, and the round-trip row was reading a
      // constant. Found by splitting the stub in two, which is the only reason the duplicate
      // became visible. A later key silently replacing an earlier one is invisible to `node
      // --check`, to the linter, and to a reader who stops at the first definition.
  ...
  return c.Chat.acceptDMInvite("!inv:hs").then((r) => {
    ...
    return d.Chat.declineDMInvite("!inv:hs").then((r2) => {
      ...
      // joined room that cannot be un-created, only left.
      {
        ...
        const account = {};
        const joinedRooms = [];
        // TWO GLOBALS OUT OF ONE SANDBOX SINCE J59. The DM surface is `MatrixAccount`'s and the
        // live `client` is `MatrixBridge`'s — one owner, and the account module asks for it. Both
        // are taken from the SAME context, so the client set below is the one the accept path reads.
        const _sb = loadInContext(["core/logger.js", "backends/backend1/ranks.js",
          "backends/backend1/matrixbridge.js", "backends/backend1/matrixaccount.js"], {
          Date, Math, JSON, setTimeout, clearTimeout, setInterval, clearInterval,
          window: {}, document: { body: { appendChild() {} } },
          localStorage: { getItem: () => null, setItem() {}, removeItem() {} },
        });
        const MBridge = _sb.MatrixBridge;
        const MB = _sb.MatrixAccount;
        const room = {
          roomId: "!inv2:hs",
        ...
        // ASSERTED RATHER THAN FALLEN BACK ON. This read `MB._setClientForTest ? ... : (MB.__client
        // = fakeClient)` — and the else branch is a silent no-op that assigns a dead property, so
        // the day the seam is renamed this guard runs the whole accept path against a null client
        // and reports whatever that produces. A fallback that cannot work is worse than no fallback.
        ok(typeof MBridge._setClientForTest === "function",
          "A: setup — the client seam must exist on MatrixBridge, which OWNS `client`; the account "
          + "module creates it and hands it over and never holds a copy (J59)",
          typeof MBridge._setClientForTest);
        MBridge._setClientForTest(fakeClient);
...
function partB() {
  ...
  const ui = fs.readFileSync(path.join(ROOT, "ui/chat.js"), "utf8")
...
function partC() {
  ...
  const ui = fs.readFileSync(path.join(ROOT, "ui/chat.js"), "utf8")
    .split("\n").map((l) => (l.trim().startsWith("//") ? "" : l)).join("\n");
  // THE INPUT MOVED, NOT WENT (ddjp_434): the search box is where a user id is typed now, and a full id
  // not in the list offers a start row that calls `_startDMByUserId` (driven in `check-chat-view` PART L2,
  // which also drives the refusals below end to end). TEXTUAL here, as it always was: the input exists
  // and reaches the handler.
  ok(/class: "dm-search"/.test(ui) && /_startDMByUserId\(userId\)/.test(ui.slice(ui.indexOf("function _dmStartRow"))),
    "C: there is an input for a user id — the DM search box, whose start row reaches the handler; `openDM` has " +
    "always refused `no-user` and `self` and no surface could reach either", "no input");
  ...
  // A dead branch cannot pass this.
  {
    ...
    // ── THE PANEL DISPATCHES THROUGH THE ADAPTER NOW (J31), SO THE ADAPTER IS THE REAL ONE ─────
    // `_startDMByUserId` called `Chat.openDM` directly and this block stubbed it. J31 routed it
    // through `Actions.perform("chat.dm", …)` — the user card already did — so the extracted
    // function needs an `Actions` in scope or it throws, which is how this guard found the change.
    //
    // THE REAL `features/actions.js` IS LOADED RATHER THAN STUBBED. A hand-written
    // `{ perform: (a, args) => Chat.openDM(args.userId) }` would be a second copy of the catalog's
    // routing, green forever regardless of what the row actually says — the exact shape
    // `check-action-routing` exists to refuse one layer up. Loading it means this block now ALSO
    // proves `chat.dm` reaches `Chat.openDM`, and it costs one line.
    //
    // `cryptoReady` IS PART OF THE SUBJECT, NOT SCAFFOLDING. `chat.dm`'s row is available only
    // while secure chat is up, so a stub without it makes every case below reject and this part
    // would fail for a reason that is not about user ids.
    const ChatStub = {
      cryptoReady: () => true,
      openDM: (u) => { asked.push(u); return Promise.resolve(
        u === "@me:hs" ? { ok: false, reason: "self" }
        : !u ? { ok: false, reason: "no-user" }
        : u === "@ghost:hs" ? { ok: false, reason: "create-failed" }
        : { ok: true, roomId: "!r:hs" }); },
    };
    const adapterSb = loadInContext(
      ["backends/backend1/ranks.js", "backends/backend1/statederiver.js",
       "backends/backend1/capabilities.js", "features/actions.js"],
      { StreamManager: { getState: () => ({ nowPlaying: null, rotation: [], settings: {} }) },
        Room: { getMyId: () => "@me:hs", getMyRank: () => 0, getMyAuthorityLevel: () => 0 },
        Chat: ChatStub, Queue: {}, Skip: {}, Reactions: {}, RoomUpgrade: {}, Date }
    );
    const ctxC = {
      Promise, console,
      renderDMPanel: () => shown.push(ctxC._dmNewError),
      _openDMConversation: (id) => opened.push(id),
      // The handler clears the DM search text on success (ddjp_434) through `refs`, as every panel function
      // reaches its elements; without it here the success path threw and the guard died with no verdict.
      refs: { dmBox: {} },
      Chat: ChatStub,
      Actions: adapterSb.Actions,
      _dmNewError: "",
    };
