...
// `floor.js` REMOVED at `ddjp_384` — over-declared. The floor here is a BOUND (a number), not a
// behaviour: every appearance is `Floor.reset()` or `Floor._setTrustedForTest(...)`, a state hook
// that bypasses adoption entirely, and the file says so itself — "`Floor.revalidate()` is never
// called here. The floor arrives already graded via `_setTrustedForTest`". The claims are all about
// `StreamManager`'s derived log being bounded and trimmed. `eventcache.js` is NOT a missing second
// subject either, checked before concluding: the WALL names two memories but EventCache appears only
// in two comments and is never loaded.
