...
// ── READ AS A POPULATION, NOT AS A FILE (the ui/ split, roles.md §5) ─────────────────────────
// The controls this guard judges are spread across the ui/ files now: the marquee pair went to
// ui/player.js with the PLAYER cluster while the back button and the DM composer stayed in
// ui/interface.js. A single-file read would have kept PASSING with half its subject gone — the
// silent loss this split was measured for — so the population is the subject and it is scanned.
// The non-vacuity assertions below are the point: a filtered check must assert it filtered to
// something, or a scan that found nothing reads exactly like a tree with nothing wrong.
const UI_DIR = path.join(ROOT, "ui");
const UI_FILES = fs.readdirSync(UI_DIR).filter((f) => f.endsWith(".js")).sort();
const ui = UI_FILES.map((f) => fs.readFileSync(path.join(UI_DIR, f), "utf8")).join("\n;\n");
...
// The population must be non-empty AND must actually contain the surfaces judged below. A
// directory that scanned to nothing, or a file list that lost the file holding the marquee,
// satisfies every negative assertion in this guard for free.
ok(UI_FILES.length >= 3, "APPLIED — ui/ must hold the split files to scan (found: " +
  UI_FILES.join(", ") + ")", UI_FILES);
ok(/function _fitMarquee\(/.test(uiCode), "APPLIED — the marquee fitter must be IN the scanned " +
  "population, or PART B's count of marquee functions is a count of nothing");
ok(/class: "back-btn/.test(uiCode), "APPLIED — the back button must be in the scanned population, " +
  "or PART A asserts about a control this guard cannot see");
ok(/class: "dm-input chat-input"/.test(uiCode), "APPLIED — the DM composer must be in the scanned " +
  "population, or PART C is vacuous");
