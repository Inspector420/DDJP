...
// SUBJECT: features/chat.js, backends/backend1/matrixaccount.js
...
// TWO FAKES OVER ONE STATE, BECAUSE J59 MADE THEM TWO MODULES. `cryptoAvailable` and
// `retryCrypto` moved to `MatrixAccount`; sending stayed on `MatrixBridge`. Injecting ONE object
// under both names would keep this green whichever module `features/chat.js` reached for — the
// stub would answer either way — so the two facades are separate and a call landing on the wrong
// one is `undefined` here. The STATE is shared, because it really is: whether crypto is up is
// what decides whether the send throws, and two copies of that would be a fake that cannot
// reproduce the failure it exists to reproduce.
function makeBridge(opts) {
  const state = { cryptoUp: opts.cryptoUp, sent: [], retryCalls: 0 };
  return {
    _state: state,
    // what `features/chat.js` reaches on the TRANSPORT
    MatrixBridge: {
    ...
    // what it reaches on the ACCOUNT module
    MatrixAccount: {
...
(async () => {
  // --- 1) crypto DOWN: send returns a status, does NOT throw, and no message goes out ---
  {
    ...
    const sb = loadInContext(["core/logger.js", "features/chat.js"], { Date, MatrixBridge: bridge.MatrixBridge, MatrixAccount: bridge.MatrixAccount });
