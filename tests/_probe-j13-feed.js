...
// THE EVENT FEED moved to ui/panels.js at ddjp_387 — but only as far as `renderFeedPanel`. Four
// roster declarations sit BELOW its banner in ui/interface.js and stayed there, so this constant
// names the file the feed is IN rather than the file its banner used to be in.
const UI_REL = "ui/panels.js";
...
function drivePanel(opts) {
  ...
  // The interim host seam (ui/base.js §host). Built FROM the stubs above rather than beside them:
  // two sets of stubs for one panel are free to disagree about what `shortName` does.
  sandbox.H = {
    el: sandbox.el, clear: sandbox.clear, shortName: sandbox.shortName,
    rankColor: sandbox.rankColor, _rosterLevel: sandbox._rosterLevel,
    _wireCardTrigger: sandbox._wireCardTrigger,
    _renderWindowedStack: sandbox._renderWindowedStack,
  };
  // ui/panels.js calls ui/queue.js DIRECTLY for the windowed stack since ddjp_388 — the seam
  // shrinking as clusters land. The stub is the same one `H` carries, not a second one.
  sandbox.QueuePanels = { _renderWindowedStack: sandbox._renderWindowedStack };
  // THE USER CARD moved to ui/roster.js at ddjp_391, so the panels call it directly rather than
  // through the host. Same stub `H` carries, not a second one.
  sandbox.Roster = { _wireCardTrigger: sandbox._wireCardTrigger };
