...
// SUBJECT: backends/backend1/history.js, core/store.js, backends/backend1/matrixbridge.js
// `matrixbridge.js` ADDED at `ddjp_384` — under-declared. PART G reads it as SOURCE at two sites and
// judges it: the rebuild block must fold SEGMENTS anchored on the checkpoints it holds
// (`History.ingest(seg, bounds[i].seed)`) rather than one run from genesis, which is what produced
// nine songs from a full room. Never loaded, and judged anyway.
// `statederiver.js` is NOT a subject here despite `SDg.derive` appearing four times: those rows
// MEASURE that anchoring recovers songs (`whole`, `oneRun`, `anchored`) to establish the principle the
// source assertion then pins. Two of the three are labelled APPLIED and the third is the
// demonstration — a measuring instrument, not the accused.
