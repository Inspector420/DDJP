...
// SUBJECT: features/queue.js, features/userqueue.js
...
const bridge = {
  ...
  async sendEvent(channel, type, content) {
    ...
    const raw = {
      ...
      sender: "@me:hs", senderRank: 20, ts: 1000 + bridge.n * 60000,
...
const sb = loadInContext(
  ["core/logger.js", "core/store.js", "backends/backend1/ranks.js", "backends/backend1/statederiver.js", "backends/backend1/streammanager.js", "core/playlistdoc.js", "features/queue.js", "features/userqueue.js"],
...
// Ingest an event authored by a MOD on a Staff+ channel — rank rides on senderRank (channel
// origin), NOT @me. Used to simulate a moderator removing me / resetting the rotation. Uses
// its OWN id counter and does NOT touch bridge.n, so bridge.n keeps counting only MY sends.
let _modSeq = 0;
function modIngest(type, content, rank) {
  _modSeq++; bridge.l++;
  const body = Object.assign({}, content, { t: type, l: bridge.l, dv: 1 });
  StreamManager.ingest({
    event_id: "$mod" + _modSeq, room_id: "!r:hs", type: "m.room.message",
    sender: "@staff:hs", senderRank: (rank || 60), ts: 90000 + _modSeq * 60000,
    content: { body: JSON.stringify(body) }, l: bridge.l
  });
}
const X = "XXXXXXXXXXX";
...
async function run() {
  ...
  // 15) MOD EVICTION — a Staff+ remove of ME (or a reset) must FALL ME OUT (offer-to-join),
  //     never auto-rejoin. My intent is kept; I return only with a deliberate Join. This is
  //     the counterpart to a true fall-out: eviction is EXTERNAL, so the never-starve refill
  //     must NOT fire — reading my emptied declared buffer as "starving" and re-declaring me
  //     was the bug this guards.
  ["JJJJJJJJJJJ","KKKKKKKKKKK","LLLLLLLLLLL"].forEach(id => UserQueue.add(url(id)));
  UserQueue.joinRoomQueue();
  expect(myPending(), ["JJJJJJJJJJJ","KKKKKKKKKKK"], "joined and declared before the eviction");
  const nBefore = bridge.n;
  modIngest("ddjp.dj.remove", { x: "@me:hs" }, 60);            // a Staff mod removes me
  if (UserQueue.isActive()) fail("a mod remove must fall me out, not keep me active");
  expect(myPending(), [], "after a mod remove I'm out of the rotation");
  expect(itemIds(), ["JJJJJJJJJJJ","KKKKKKKKKKK","LLLLLLLLLLL"], "eviction keeps my intent (SAFE-DROP)");
  settle();                                                    // fire any pending timer
  if (UserQueue.isActive()) fail("must STILL be out after the settle — no auto-rejoin");
  expect(myPending(), [], "still out after settle — the never-starve refill must not re-declare an evicted DJ");
  if (bridge.n !== nBefore) fail("an evicted DJ must not put any re-declare on the wire", bridge.n - nBefore);
  UserQueue.joinRoomQueue();                                   // I return only by choosing to Join
  expect(myPending(), ["JJJJJJJJJJJ","KKKKKKKKKKK"], "an explicit Join re-declares my kept songs");

  // 15b) A reset evicts everyone the same way — fall out, keep intent, no auto-rejoin.
  modIngest("ddjp.dj.reset", {}, 80);                          // reset is High-Staff+
  if (UserQueue.isActive()) fail("a reset must fall me out too");
  expect(myPending(), [], "after a reset I'm out of the rotation");
  expect(itemIds(), ["JJJJJJJJJJJ","KKKKKKKKKKK","LLLLLLLLLLL"], "reset keeps my intent");
  settle();
  if (UserQueue.isActive()) fail("still out after a reset + settle — no auto-rejoin");
  expect(myPending(), [], "reset then settle leaves me out until I choose to Join");
  UserQueue.clearQueue();

  // 15c) Eviction BEATS the on-the-decks auto-rejoin. A mod removes me WHILE my song is
  //      still the one playing. Running dry in that state lets an add re-enter me (scenario
  //      6); an eviction must NOT — it requires a deliberate Join even mid-song.
  ["MMMMMMMMMMM","NNNNNNNNNNN"].forEach(id => UserQueue.add(url(id)));
  UserQueue.joinRoomQueue();
  expect(myPending(), ["MMMMMMMMMMM","NNNNNNNNNNN"], "joined with two songs");
  await play();   // M plays → I'm now-playing M, still in the rotation with N
  if (StreamManager.getState().nowPlaying.dj !== "@me:hs") fail("I should be the now-playing DJ (M)");
  expect(myPending(), ["NNNNNNNNNNN"], "N is my remaining buffered song while M plays");
  modIngest("ddjp.dj.remove", { x: "@me:hs" }, 60);   // a mod removes me mid-song
  if (StreamManager.getState().nowPlaying.dj !== "@me:hs") fail("my song M should keep playing after the remove");
  if (UserQueue.isActive()) fail("a mod remove must fall me out even while I'm now-playing");
  const nMid = bridge.n;
  UserQueue.add(url("OOOOOOOOOOO"));   // adding while STILL now-playing must not slip me back in
  if (UserQueue.isActive()) fail("adding after an eviction must NOT auto-rejoin, even mid-song");
  expect(myPending(), [], "still out — the eviction suppresses the on-the-decks auto-rejoin");
  if (bridge.n !== nMid) fail("an evicted add must not re-declare on the wire", bridge.n - nMid);
  UserQueue.joinRoomQueue();   // only a deliberate Join re-enters me
  if (!UserQueue.isActive()) fail("a deliberate Join after eviction re-activates me");
  UserQueue.clearQueue();
  await play();   // advance past M to leave a clean state

  console.log("[feature-flow] PASS — one intent list feeds and refills; SAFE-DROP by entry-id (3 copies, one plays, the exact played entry drops and the other two survive); a played song leaves intent while a kick/leave/reorder never does; auto-rejoins while still now-playing; stays out after a true fall-out until an explicit re-join; a mod remove/reset EVICTS (falls out, keeps intent, no auto-rejoin — even mid-song the on-the-decks auto-rejoin is suppressed; return only by Join); malformed paste ids are cleaned or rejected; movement buttons are plain reorders (local ones pure, top-two ones re-declare); each edit is minimal on the wire (top-two reorder 1, remove 2 — the shift-up is derived, not sent); Clear wipes the whole list and falls out; and the queue fills to a cap then refuses without dropping anything");
  process.exit(0);
}
