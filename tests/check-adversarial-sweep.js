...
// SUBJECT: backends/backend1/floor.js, backends/backend1/trustpolicy.js, backends/backend1/statederiver.js
// NOT A SUBJECT: `vouch.js`. It is in the load list so the sandbox builds, and it was declared here
// (`ddjp_362`) before the grammar rule. Nothing in this file calls the module: the vouching ARITHMETIC
// the sweeps drive is `TrustPolicy.owesVouch` / `protectedFor`, which live in `trustpolicy.js`, and the
// dependency runs vouch -> TrustPolicy rather than the other way. `Checkpoint` is only `attach`ed and
// never sealed, so the one arrow that does reach `Vouch` is never traversed, and `floor` asks it
// nothing by design (`SPINE.md` §6). Re-derive before restoring: `grep -in vouch` matches 12 times
// here and NOT ONCE on the module — the load list, a `vouchTable` SETTINGS field, `TrustPolicy.owesVouch`,
// and prose. This is the row J56 names as the demonstration that a load list is not a subject.
