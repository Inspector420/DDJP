...
// SUBJECT: backends/backend1/vouch.js, backends/backend1/matrixbridge.js
// `consensushash.js` REMOVED at `ddjp_384` — over-declared. It occurs FOUR times and not one is a
// claim about it: the destructure; `Vouch.commitFor(real) !== ConsensusHash.contentHash(real)`, where
// Vouch is the accused and the hash is the STANDARD it must not equal; a `pHash` fixture builder; and
// a `B5: APPLIED` SETUP CONTROL confirming the fixture reaches the hasher at all. Measuring
// instrument, fixture builder and setup control — all three vehicle categories, none a subject.
// Positive control before trusting that count: `grep -ic hash` matches 39 times in this file, so the
// query is live and the absence is the module's rather than the matcher's.
// `matrixbridge.js` STAYS: it is read as SOURCE twice and judged both times — the emitter must stamp
// `Vouch.commitFor` rather than a hash of its own choosing, and transport must reference no request
// type at all so the surface cannot be revived by wiring alone.
