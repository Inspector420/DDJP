...
// SUBJECT: backends/backend1/checkpointformat.js
// `consensushash.js` REMOVED at `ddjp_384` — over-declared, and it is a MEASURING INSTRUMENT rather
// than a subject. Its only reach is `statePrint = (st) => CH.contentHash(...)`, used to compare two
// derived states against each other — so both sides of every such assertion move together and a
// change there cannot falsify one. Its one other appearance is the PART A CONTROL proving
// `statePrint` discriminates at all. The envelope claims — that a file round-trips without changing
// the artefact it wraps — are asserted on `CF.filePrint` / `CF.readFile`, which are
// `checkpointformat.js`'s own and remain the subject.
//
