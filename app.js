...
// login/logout/create-room handlers. No logic change from the former inline block.
  ...
  // ── BIND THE BOOTSTRAP ENGINE, FIRST THING (J23) ────────────────────────────────────────────
  // The four interface names are shells until something binds (`core/backends.js`), and the whole
  // lobby runs before any room is entered: session restore, the encryption gate, login and room
  // discovery all reach `MatrixAccount` and `MatrixBridge` with no room in hand. §4 is what makes
  // this honest rather than a hardwire — discovery and account are shared by every near-term mode,
  // so this binds the transport everyone shares, and `Room.join` rebinds to the room's own engine.
  //
  // No backend is NAMED here. The bootstrap engine is the one that registered itself as such, so
  // the rule that nothing outside the registry names a concrete backend holds in this file too.
  // The bootstrap bind is NOT here any more — it has to happen before `ui/` and `features/` load,
  // and this file is the last script on the page. It is `core/bind-bootstrap.js`, which sits
  // directly after the engines in `index.html`. Left as a note rather than a second call, because
  // two callers for one thing is how the first one stops being where people look.

  // ── OFFER THE ROOM TYPES, IF THERE IS A CHOICE ──────────────────────────────────────────────
  // Drawn from what the registry describes, so no engine is named here. The row stays hidden while
  // one engine is ready: a dropdown with a single option is a decision a person cannot make, and
  // this way the create screen is untouched until a second engine is actually finished.
  function _fillRoomTypes() {
    const row = document.getElementById("row-room-type");
    const sel = document.getElementById("input-room-type");
    const note = document.getElementById("room-type-blurb");
    if (!row || !sel) return;
    let types = [];
    try { types = (Room.availableTypes && Room.availableTypes()) || []; } catch (e) { types = []; }
    if (types.length < 2) { row.style.display = "none"; sel.innerHTML = ""; return; }
    sel.innerHTML = "";
    ...
    const show = () => {
      const hit = types.filter((t) => t.id === sel.value)[0];
      if (note) note.textContent = hit ? hit.blurb : "";
    };
    sel.onchange = show;
    show();
    row.style.display = "";
  ...
  async function ensureEncryption() {
    let st;
    try { st = await MatrixAccount.encryptionStatus(); }
    ...
    await new Promise((resolve) => {
      const enterFlow = () => Interface.showEnterRecoveryKey({
        onUnlock: async (key) => { await MatrixAccount.unlockEncryption(key); resolve(); },
        onForgot: () => warnFlow(),
        onLogout: async () => { try { await MatrixAccount.logout(); } catch (e) {} location.reload(); },
      });
      ...
      async function createFlow() {
        let key;
        try { key = await MatrixAccount.generateRecoveryKey(); }
        catch (e) { Logger.error("Generate recovery key failed: " + (e && e.message)); resolve(); return; }
        Interface.showSaveNewKey({
          recoveryKey: key,
          confirmMatch: (typed) => MatrixAccount.confirmRecoveryKeyMatches(typed),
          onConfirm: async () => { await MatrixAccount.commitNewRecoveryKey(); resolve(); },
  ...
  (async () => {
    ...
    if (MatrixAccount.hasPendingSsoLogin()) {
      try {
        await MatrixAccount.completeSsoLogin();
        ...
        Interface.renderRoomList(Room.scanDDJPRooms()); _fillRoomTypes();
      } catch (err) {
    ...
    const session = await MatrixAccount.restoreSession();
    if (session) {
      await Store.account.setUser(MatrixBridge.getUserId());
      await MatrixBridge.start();
      await MatrixBridge.waitForSync();
  ...
  document.getElementById("btn-login").addEventListener("click", async () => {
    ...
    try {
      ...
      await _withTimeout(MatrixAccount.login(homeserver, username, password), 20000, "Login timed out — check the homeserver address.");
      await Store.account.setUser(MatrixBridge.getUserId());
      await MatrixBridge.start();
      await MatrixBridge.waitForSync();
      await ensureEncryption();
  ...
  if (ssoBtn) ssoBtn.addEventListener("click", async () => {
    ...
    try {
      const flows = await MatrixAccount.getLoginFlows(homeserver);
      if (!flows.sso) {
      ...
      await MatrixAccount.startSsoLogin(homeserver);   // navigates away
    } catch (err) {
  ...
  function openAccounts() {
    Interface.showAccounts({
      accounts: MatrixAccount.listAccounts(),
      activeUserId: MatrixAccount.getActiveUserId(),
      hasSession: (uid) => MatrixAccount.hasStoredSession(uid),
      onSwitch: (uid) => { if (MatrixAccount.switchAccount(uid)) location.reload(); },
      onForget: async (uid) => {
        try { await MatrixAccount.forgetAccount(uid); } catch (e) { Logger.warn("Forget (auth) failed: " + (e && e.message)); }
  ...
  async function attemptCreate(name, file) {
    ...
    try {
      // The type the person picked, or nothing when the row was never drawn — `Room.create`
      // then uses the engine this client is running, which is what every caller got before.
      // A save FILE decides its own type from its mode marker, so the picker is not consulted.
      const typeSel = document.getElementById("input-room-type");
      const picked = (typeSel && typeSel.value) ? typeSel.value : null;
      const room = file ? await Room.createFromFile(name, file) : await Room.create(name, picked);
