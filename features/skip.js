...
const Skip = (() => {
  ...
  // ── AN OPTIONAL REASON TOKEN ─────────────────────────────────────────────────────────────
  // `reason` is a token from the reducer's small closed list, not free text and not a sentence.
  // Passed only by a caller that has one — the bot's replay-cooldown sweep today. Absent is the
  // ordinary case and is what a person clicking Skip sends, so the button's behaviour is unchanged
  // by this parameter existing.
  //
  // WHY THE REDUCER NEEDS IT: a skip pushes nothing, but the playing it ENDS already has a history
  // row, and `k` is what marks that row as "cut short by the replay rule" so the rule does not
  // measure its own skip as a fresh play.
  async function skip(reason) {
    ...
    // The rule lives in Capabilities, not here — Skip asks, it does not decide.
    const verdict = Capabilities.can("dj.skip", StreamManager.getState(), { myId: me, myRank: Room.getMyRank() });
    if (!verdict.permitted) {
      Logger.warn("Skip: " + verdict.reason);
    ...
    const body = { p: targetPi };
    if (typeof reason === "string" && reason) body.k = reason;
    await MatrixBridge.sendEvent(eventsChannel, "ddjp.dj.skip", body);
  ...
  function _waitForAdvance(beforePi) {
    ...
    return new Promise((resolve) => {
      ...
      const finish = (result) => {
        ...
        StreamManager.off("ddjp.media.skip", check);
      ...
      // ddjp.media.skip — THE AVAILABILITY ESCAPE MOVES THE ROOM TOO. This watcher asks one
      // question: did now-playing actually move? The escape moves it. Without this subscription
      // nothing woke the check, the 4s window expired, and skip() reported
      // { ok:false, "missed — someone else's skip or play landed first" } about a room that HAD
      // advanced. A legal answer that is wrong, which is this codebase's whole failure signature —
      // and it does not stop being that because the value lands in a UI string.
      StreamManager.on("ddjp.media.skip", check);
