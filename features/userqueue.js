...
const UserQueue = (() => {
  ...
  let _dirty = false;       // set once the user edits; a late hydrate must not clobber edits
  ...
  function _persist() {
    _dirty = true;   // an edit happened; a still-pending hydrate must not overwrite it
    if (roomSpaceId) Store.queue.persist(roomSpaceId, { stack: stack, active: active });
  }
  function _notify() {
  ...
  function init(spaceId) {
    ...
    _dirty = false;
    const gen = ++_loadGen;
    // Hydrate the saved stack asynchronously (Store.queue is now IndexedDB-backed).
    // Apply ONLY if this is still the current room and the user hasn't started
    // editing in the (sub-frame) load window — so a late resolve can't wipe edits
    // or restore a stack into the wrong room.
    Promise.resolve(Store.queue.load(spaceId)).then((saved) => {
      if (gen !== _loadGen || _dirty || stack.length || active) return;
      if (saved && Array.isArray(saved.stack)) { stack = saved.stack; active = !!saved.active; _notify(); }
    }).catch(() => {});
    inFlight = [];
  ...
  function destroy() { _cleanup(); roomSpaceId = null; stack = []; inFlight = []; active = false; }
