...
// (docs/main/07-security.md / docs/main/02-architecture.md / check-no-media.js) forbids carrying media as
...
// calls MUST live in backends/backend1/matrixbridge.js (the only place the SDK is
...
const Media = (() => {
  ...
  function getAvatarUrl(userId) { return MatrixAccount.getAvatarUrl ? MatrixAccount.getAvatarUrl(userId) : null; }
  function onAvatarChange(fn)   { if (MatrixAccount.onAvatarChange) MatrixAccount.onAvatarChange(fn); }
  function offAvatarChange(fn)  { if (MatrixAccount.offAvatarChange) MatrixAccount.offAvatarChange(fn); }
  function uploadAvatar(file) {
    if (!MatrixAccount.uploadAvatar) return Promise.resolve({ ok: false, reason: "avatars unavailable" });
    return MatrixAccount.uploadAvatar(file);
  }
