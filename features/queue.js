...
const Queue = (() => {
  ...
  // ── IT SAID "WILL RETRY WHEN LIVE" AND NOTHING RETRIED ────────────────────────────────────────
  // Reported from a live room: a song offered while the session was not live was logged as deferred
  // and then **dropped**. The message named a mechanism that did not exist — every caller simply
  // returned `{ retrying: true }` and nobody ever looked at it again. A person sees a click that
  // does nothing, and the log says the opposite of what happened.
  //
  // It bites hardest when the tab is not focused, because that is when a session goes quiet — so
  // "add a song, then go and look at something else" loses the song.
  //
  // NOW IT ACTUALLY WAITS. `onAuthorReady` is the hook the transport already exposes for exactly
  // this, and it is the one `backends/backend2/runner.js` uses to hold its own writes until the
  // clock has caught up. Every caller here is `async` and already awaits, so waiting costs them
  // nothing. The timeout is the honest part: if liveness never comes back the act is refused with a
  // reason rather than hanging a promise nobody resolves.
  const LIVE_WAIT_MS = 15000;
  ...
  function _mayWrite(what) {
    try {
      ...
      if (fit && fit.ok === false) {
        Logger.info("Queue: holding " + what + " until this client is live again — " + fit.reason);
  ...
  // Resolves `{ ok: true }` once this client may author, or a refusal if it does not become live
  // within the window. Never rejects: a caller's `if (!w.ok) return w` stays exactly as it was.
  // `first` is the refusal the caller already has, so THE LIVE PATH NEVER BECOMES ASYNCHRONOUS.
  // An earlier version awaited unconditionally and moved every send one microtask later; guards that
  // call `UserQueue.joinRoomQueue()` without awaiting then asserted before the send had happened.
  // A fix that changes when a working path runs is a fix that breaks the working path.
  function _awaitWritable(what, first) {
    if (!first || first.ok) return Promise.resolve(first || { ok: true });
    if (typeof MatrixBridge.onAuthorReady !== "function") return Promise.resolve(first);
    return new Promise((resolve) => {
      let done = false;
      const finish = (v) => { if (!done) { done = true; resolve(v); } };
      const timer = setTimeout(() => {
        Logger.warn("Queue: gave up holding " + what + " — this client did not become live");
        finish({ ok: false, reason: first.reason, retrying: false });
      }, LIVE_WAIT_MS);
      try {
        MatrixBridge.onAuthorReady(() => {
          const again = _mayWrite(what);
          if (!again.ok) return;               // a spurious wake; keep waiting for a real one
          clearTimeout(timer);
          Logger.info("Queue: live again — sending " + what + " now");
          finish({ ok: true });
        });
      } catch (e) { clearTimeout(timer); finish(first); }
    });
  }

  async function submitSong(videoId, url) {
    { const w = _mayWrite("a song"); if (!w.ok && !(await _awaitWritable("a song", w)).ok) return w; }
  ...
  async function join() {
    { const w = _mayWrite("a join"); if (!w.ok && !(await _awaitWritable("a join", w)).ok) return w; }
    if (!eventsChannel) return;
    await MatrixBridge.sendEvent(eventsChannel, "ddjp.dj.join", {});
  }
  async function leave() {
    { const w = _mayWrite("a leave"); if (!w.ok && !(await _awaitWritable("a leave", w)).ok) return w; }
    if (!eventsChannel) return;
    await MatrixBridge.sendEvent(eventsChannel, "ddjp.dj.leave", {});
  }
  async function move(userId, afterUserId) {
    { const w = _mayWrite("a move"); if (!w.ok && !(await _awaitWritable("a move", w)).ok) return w; }
  ...
  async function remove(userId) {
    { const w = _mayWrite("a remove"); if (!w.ok && !(await _awaitWritable("a remove", w)).ok) return w; }
    if (!eventsChannel || !userId) return;
  ...
  async function strike(userId, videoId) {
    { const w = _mayWrite("a strike"); if (!w.ok && !(await _awaitWritable("a strike", w)).ok) return w; }
  ...
  async function reset() {
    { const w = _mayWrite("a reset"); if (!w.ok && !(await _awaitWritable("a reset", w)).ok) return w; }
    if (!eventsChannel) return;
  ...
  async function reorder(videoIds) {
    { const w = _mayWrite("a reorder"); if (!w.ok && !(await _awaitWritable("a reorder", w)).ok) return w; }
  ...
  async function undeclare(videoId) {
    { const w = _mayWrite("an undeclare"); if (!w.ok && !(await _awaitWritable("an undeclare", w)).ok) return w; }
    if (!eventsChannel || !videoId) return;
