...
const ServerClock = (() => {
  ...
  // ── A HIDDEN TAB IS NOT A MEASUREMENT ────────────────────────────────────────────────────
  // REPORTED FROM A ROOM: the bot acting thirty seconds to two minutes late, on top of its sweep
  // period. Measured cause: a sample is `localAtReceive - serverTs`, which cannot tell a clock
  // difference from DELIVERY DELAY. Browsers throttle timers and delivery in a hidden tab, so a
  // backgrounded client wakes to a clump of late arrivals and reads their lateness as the time
  // being different. `_live()` does not catch it — a throttled tab is still LIVE.
  //
  // Then it PERSISTS: the median is over the last twelve samples, so in a quiet room a poisoned
  // window survives until twelve good ones push it out. That persistence, not the inaccuracy, is
  // what produced a lag measured in minutes.
  //
  // SO A HIDDEN TAB LEARNS NOTHING. Not "learns and corrects later" — there is no way to correct a
  // sample after the fact, because nothing records how late it was. Refusing is the only honest
  // option, and it costs nothing: a hidden tab is not driving a playhead anybody is watching.
  //
  // THIS ONLY EVER DISCARDS, which is the property that makes it safe to ship without a browser to
  // watch. It cannot move the offset, and it certainly cannot move it FORWARD — a clock running
  // fast is the direction that makes clients send advances the reducer then refuses, and makes the
  // bot remove people early. Nothing here can do that.
  function _hidden() {
  ...
  // ── AND WHAT WAS LEARNED WHILE HIDDEN IS DROPPED ON THE WAY BACK ─────────────────────────
  // The guard above stops NEW poison. This clears what was already in the window when the tab went
  // away — samples taken in the moments before the browser throttled it, which are the latest and
  // therefore the most polluted. Without this, becoming visible leaves the old median in place
  // until twelve fresh samples arrive, which in a quiet room is the whole of the reported lag.
  //
  // `_offset` IS DELIBERATELY KEPT while the window refills. Zeroing it would snap every playhead
  // in the room to the raw local clock for as long as it takes to relearn — a visible jump, traded
  // for a stale-but-close number. The old estimate is the better of the two until a new one exists.
  function _onShown() {
    if (_hidden()) return;
    if (_samples.length === 0) return;
    _samples = [];
    Logger.debug("ServerClock: back in the foreground — samples taken around the throttle are "
      + "discarded and the offset is relearned from fresh traffic; the last estimate stands "
      + "until then");
  }
  ...
  function init() {
    if (!_subscribed) {
      ...
      // Subscribed here rather than at load: `init` is the point the module is wired, and a
      // listener added at load would outlive a `destroy`.
      try {
        if (typeof document !== "undefined" && document.addEventListener) {
          document.addEventListener("visibilitychange", _onShown);
        }
      } catch (e) { /* no document: the sample guard above still holds */ }
      _subscribed = true;
    }
  }
  function destroy() {
