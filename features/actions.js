...
const Actions = (() => {
  ...
  const CATALOG = {
    ...
    // THE @ BUTTON (ddjp_431), AND `verb: null` IS A DECISION, as for `chat.dm` above. Putting somebody's
    // Matrix ID in your OWN chat box is not an act anybody adjudicates — no reducer folds it and no
    // homeserver rule applies — so a rank gate would report permitted against nothing. `Chat.mentionText`
    // hands back the ID (or refuses a non-ID); the card closes and the chat panel inserts it
    // (`ChatPanels.insertMention`).
    "chat.mention":  { verb: null,            run: (a) => Chat.mentionText(a.userId), label: () => "@" },
    "room.invite":   { verb: "room.invite",   run: (a) => Room.invite(a.userId) },
    // `onProgress` IS A FEATURE PARAM AND HAS TO SURVIVE THE ADAPTER (J31). This row read
    // `run: () => RoomUpgrade.upgrade()` while the UI called `RoomUpgrade.upgrade(cb)` directly
    // with a progress callback — so routing the button through `perform` without fixing the row
    // would have silently dropped the callback and frozen the upgrade progress bar at "Starting…"
    // for the whole batch. Nothing would throw; the rooms would still be created. **That is a
    // behaviour change hiding inside a change filed as "uniformity only, no behaviour change"**,
    // and it is the reason a routing job has to check each row against its call site rather than
    // assume the row was written for one. The header already says `args` doubles as the capability
    // target and the feature-call params; a callback is the second of those, and `Capabilities.can`
    // ignores target fields it does not name.
    "room.upgrade":  { verb: "room.upgrade",  run: (a) => RoomUpgrade.upgrade(a && a.onProgress),  label: () => "Upgrade" },
  };
